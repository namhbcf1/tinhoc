...
function buildUpperFullName(ho, tenDem, ten) {
...
function sanitizeStudentTextField(field, value) {
...
function isSyntheticTestStudentCccd(value) {
    const normalized = sanitizeStudentTextField('cccd', value);
    return /^(?:00[1-9]|001[0-9])$/.test(normalized);
}
export function isAcceptedStudentLoginSecret(cccd, storedPhone, providedSecret) {
...
function derivePublicBaseUrl(c) {
...
export function normalizeStudentGender(input, fallback = null) {
    ...
    if (folded === 'nu' || folded === 'female' || folded === 'f')
        return 'Nữ';
    if (folded === 'khac' || folded === 'other' || folded === 'x')
        return 'Khác';
    throw new Error('Giới tính không hợp lệ. Chỉ hỗ trợ Nam, Nữ hoặc Khác.');
}
export async function enrichStudentWithImages(c, student) {
    if (!student)
        return student;
    const s = { ...student };
    s.ho = toUpperVi(s.ho);
    ...
    const makeR2Url = (key) => key ? `${baseUrl}/students/image/${encodeURIComponent(key)}` : null;
    const isAbsoluteUrl = (value) => /^https?:\/\//i.test(value);
    const isLikelyR2Key = (value) => value.includes('/');
    const resolveStoredImage = async (value) => {
        const normalized = normalizeWhitespace(value);
        if (!normalized)
            return null;
...
export async function uploadImage(c, file) {
    if (file.size > 50 * 1024 * 1024)
        throw new Error('Ảnh quá lớn. Kích thước tối đa là 50MB');
...
export async function registerStudent(c, data) {
    ...
    if (existingByCCCD)
        throw new Error('Số CCCD/CMT đã được đăng ký. Vui lòng kiểm tra lại!');
    ...
    if (existingByContact.length > 0) {
        if (existingByContact[0].sdt === normalizedProbePhone)
            throw new Error('Số điện thoại đã được đăng ký.');
        if (existingByContact[0].email === normalizedProbeEmail)
...
export async function getStudentsList(c, limit, offset, page = 1) {
    ...
    const data = await Promise.all(list.map(async (s) => {
        ...
        return { ...enriched, registrations: regs.map((r) => ({ ...r, class_type: r.class_type || 'hoc' })) };
    }));
    return {
        data,
        meta: {
...
export async function searchStudents(c, query) {
    if (query.length < 2)
        throw new Error('Từ khóa phải ít nhất 2 ký tự');
...
export async function getStudentEditHistory(c, id, limit, offset) {
...
export async function getStudentByCCCD(c, cccd) {
    if (cccd.includes('/'))
...
export async function updateStudentByCCCD(c, data) {
    const currentCCCD = sanitizeStudentTextField('cccd', data.current_cccd || data.cccd || '');
    if (!currentCCCD)
        throw new Error('Thiếu CCCD học viên');
    ...
    for (const field of STUDENT_SELF_EDITABLE_FIELDS) {
        if (data[field] !== undefined)
            updateData[field] = sanitizeStudentTextField(field, data[field]);
    }
    if (data.ngay_sinh !== undefined)
        updateData.ngay_sinh = data.ngay_sinh ? formatDate(data.ngay_sinh) : null;
    if (data.ngay_cap_cccd !== undefined)
        updateData.ngay_cap_cccd = data.ngay_cap_cccd ? formatDate(data.ngay_cap_cccd) : null;
    if (data.gioi_tinh !== undefined)
        updateData.gioi_tinh = normalizeStudentGender(data.gioi_tinh, student.gioi_tinh);
    ...
    if (updateData.ho !== undefined || updateData.ten !== undefined || updateData.ten_dem !== undefined) {
        updateData.ho_ten_full = buildUpperFullName(updateData.ho ?? student.ho, updateData.ten_dem ?? student.ten_dem ?? '', updateData.ten ?? student.ten);
    ...
    const changedFields = Object.keys(updateData).filter((field) => {
        if (skipLogFields.has(field))
            return false;
        return !valuesEqual(student[field], updateData[field]);
    });
    if (changedFields.length > 0) {
        await Promise.all(changedFields.map((field) => (StudentRepo.logStudentEditHistory(c.env.DB, {
...
export async function updateStudentAdmin(c, id, data) {
    ...
    if (normalizedIncomingCCCD && normalizedIncomingCCCD !== sanitizeStudentTextField('cccd', existing.cccd)) {
        if (await StudentRepo.findStudentByCCCD(c.env.DB, normalizedIncomingCCCD))
            throw new Error('Số CCCD đã được sử dụng bởi học viên khác');
    }
    const updateData = {};
    ...
    if (data.ngay_sinh)
        updateData.ngay_sinh = formatDate(data.ngay_sinh);
    if (data.ngay_cap_cccd)
        updateData.ngay_cap_cccd = formatDate(data.ngay_cap_cccd);
    if (data.gioi_tinh)
        updateData.gioi_tinh = normalizeStudentGender(data.gioi_tinh, existing.gioi_tinh);
...
export async function deleteStudentAdmin(c, id) {
    const existing = await StudentRepo.getStudentById(c.env.DB, id);
    if (!existing)
        throw new Error('Không tìm thấy học viên');
...
export async function normalizeAllStudentsUppercase(c, dryRun = false) {
