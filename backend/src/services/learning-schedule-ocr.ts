export type LearningImportRowStatus = 'ready' | 'needs_review' | 'duplicate';

export interface ParsedLearningScheduleRow {
  row_id: string;
  source_text: string;
  ...
  note: string | null;
  status: Exclude<LearningImportRowStatus, 'duplicate'>;
  warnings: string[];
  corrections: string[];
...
const OCR_DIGIT_FIXES: Array<[RegExp, string, string]> = [
  [/O/g, '0', 'O->0'],
  [/o/g, '0', 'o->0'],
  [/Q/g, '0', 'Q->0'],
  [/D/g, '0', 'D->0'],
...
function applyDigitFixes(raw: string): { normalized: string; corrections: string[] } {
  let normalized = raw;
...
function normalizeYear(rawYear: string, anchorYear?: number): { year: number | null; corrected: boolean; warning?: string; correction?: string } {
...
function normalizeDateToken(rawDate: string, anchorYear?: number): DateParseResult {
  ...
  if (yearResult.correction) corrections.push(yearResult.correction);
...
function normalizeTimeToken(rawToken: string): TimeParseResult {
  const warnings: string[] = [];
  const corrections: string[] = [];
...
export function parseLearningScheduleText(rawText: string, options?: { anchorYear?: number }): ParsedLearningScheduleRow[] {
  ...
  const rows: ParsedLearningScheduleRow[] = [];
...
export function buildSessionDuplicateKey(sessionDate: string | null | undefined, startTime: string | null | undefined, endTime: string | null | undefined): string {
