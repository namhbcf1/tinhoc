 ...
 * @param {string} impersonateEmail - Email của user cần impersonate
 * @param {string[]} scopes - OAuth scopes
 */
async function generateJWT(clientEmail, privateKey, impersonateEmail, scopes) {
 ...
 * @param {string} scheduleRule - VD: "WEEKLY:1,3,5" (Thứ 2, 4, 6)
 * @param {string} endDate - Ngày kết thúc (optional)
 * @returns {string|null} - RRULE string hoặc null nếu single event
 */
 ...
 * @param {string} scheduleTime - VD: "19:00-21:00"
 * @param {string} startDate - VD: "2026-01-22"
 ...
 * @param {Object} env - Cloudflare env với secrets
 * @param {Object} classData - Thông tin lớp học
 * @returns {Object} - { eventId, meetLink, recurrence, htmlLink }
 */
 ...
 * @param {Object} updateData - Dữ liệu cần update (chỉ class_name, description)
 */
 ...
 * @returns {Object|null} - { eventId, meetLink } hoặc null
 */
export async function findEventByClassName(env, className, startDate) {
  ...
  const matched = data.items.find(event => 
