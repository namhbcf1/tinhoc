...
function parseIntegerEnv(value, defaultValue, min, max) {
...
function sanitizePromptText(value, maxLength = 80) {
    return String(value || '')
        .replace(/[\r\n\t]+/g, ' ')
        .replace(/[{}[\]<>]/g, ' ')
        .replace(/\s+/g, ' ')
...
function bufferToDataUrl(bytes, mimeType = 'image/jpeg') {
...
function dedupeWarnings(...groups) {
    const merged = groups
        .flatMap((group) => group || [])
...
function buildPipelineObjectKey(processingLogId, stage, suffix, contentType) {
    const extension = detectExtension(contentType);
    return `cccd-uploads/photo_3x4/pipeline/${processingLogId}/${stage}-${suffix}.${extension}`;
}
function createFallbackEvaluation(score, warnings, usable = true) {
    return {
        usable,
        score,
...
function normalizeIdentityProfile(raw, genderHint) {
...
function normalizeVariantEvaluation(raw) {
    const data = raw && typeof raw === 'object' ? raw : {};
...
async function updateProcessingLog(env, logId, fields) {
...
async function fetchProcessingLog(env, logId) {
...
async function deleteVariants(env, logId) {
    await env.DB.prepare('DELETE FROM photo_3x4_variants WHERE processing_log_id = ?').bind(logId).run();
}
async function insertVariant(env, logId, slot, variant, recommended) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(logId, slot, variant.imageId, variant.generationMode, variant.score, recommended ? 1 : 0, JSON.stringify(variant.warnings), JSON.stringify(variant.validationResult), variant.promptProfile).run();
    return Number(inserted.meta.last_row_id || 0) || 0;
}
async function storeR2Blob(env, key, blob, metadata = {}) {
...
async function fetchReferenceBlob(sourceUrl, options) {
    const { width, height, fit = 'cover', gravity = 'face', trim, quality = 86, sharpen, brightness, contrast, } = options;
...
async function fetchVisionReviewBytes(url) {
...
async function runDeterministicTransform(sourceUrl) {
    return await fetchBlob(sourceUrl, {
        cf: {
            image: {
...
async function maybeAgreeToModel(env, model) {
    try {
...
async function describeIdentityForPortrait(env, imageBytes, contentType, genderHint) {
    try {
        ...
        const rawPayload = response?.response ?? response;
...
function buildGenderInstruction(gender) {
    if (gender === 'male') {
...
function buildGenerativePrompt(identity, profile, genderHint) {
    const hintedGender = normalizeGenderHint(genderHint);
...
async function fetchStyleReferenceBlob(env, sourceUrl) {
...
async function buildReferenceBlobs(env, sourceUrl) {
    ...
    return [medium, tight, wide, styleReference].filter(Boolean);
}
async function generatePortraitWithAI(env, references, identity, profile, genderHint) {
    const model = getPrimaryModel(env);
    await maybeAgreeToModel(env, model);
    const buildMultipartResponse = (blobs) => {
    ...
    const attemptGenerate = async (blobs) => {
...
async function polishPortraitOutput(sourceUrl) {
    return await fetchBlob(sourceUrl, {
        cf: {
            image: {
                width: DETERMINISTIC_WIDTH,
...
async function evaluatePortraitVariant(env, identity, candidateBytes, candidateContentType) {
    try {
        await maybeAgreeToModel(env, VISION_MODEL);
...
async function buildFallbackVariant(env, identity, sourceVision, previewUrl, imageId, generationMode, promptProfile, defaultScore, extraWarnings) {
    try {
        const candidateVision = await fetchBytes(previewUrl);
        const evaluation = await evaluatePortraitVariant(env, identity, candidateVision.bytes, candidateVision.contentType);
        return {
            imageId,
            generationMode,
            promptProfile,
...
async function generateAiVariant(env, processingLogId, sourceVision, previewBaseOrigin, references, identity, profile, metadata) {
...
async function markFailed(env, processingLogId, message, extra = {}) {
...
export async function enqueuePhoto3x4Pipeline(env, message) {
...
export async function runPhoto3x4Pipeline(env, message) {
    ...
    try {
        ...
        const identity = await describeIdentityForPortrait(env, sourceVision.bytes, sourceVision.contentType, genderHint);
        await updateProcessingLog(env, processingLogId, {
        ...
        const aiVariantPromises = aiEnabled && generativeEnabled
            ? PROMPT_PROFILES.slice(0, variantCount).map(async (profile) => {
                try {
                    return await generateAiVariant(env, processingLogId, sourceVision, previewBaseOrigin, references, identity, profile, { studentId, genderHint });
                }
                catch (error) {
                    return {
                        error: `Khong tao duoc bien the ${PROMPT_PROFILE_LABELS[profile.id]}: ${String(error?.message || error || 'unknown')}`,
                    };
                }
            })
            : [];
        ...
        if (pendingVariants.length < variantCount && candidatePreviewUrl) {
            pendingVariants.push(await buildFallbackVariant(env, identity, sourceVision, candidatePreviewUrl, candidateUpload.imageId, 'transform_fallback', 'transform_fallback', 64, ['Dung ban AI can lai khi cac phuong an sinh anh chua on dinh']));
        }
        if (pendingVariants.length < variantCount && originalPreviewUrl) {
            pendingVariants.push(await buildFallbackVariant(env, identity, sourceVision, originalPreviewUrl, originalImageId, 'original_fallback', 'original_fallback', 44, ['Giu anh goc de hoc vien tu doi chieu']));
        }
        ...
        if (finalVariants.length === 0) {
            await markFailed(env, processingLogId, 'Khong tao duoc bat ky phuong an anh 3x4 nao co the hien thi cho hoc vien.', {
                source_image_id: originalImageId,
        ...
        await updateProcessingLog(env, processingLogId, {
            ...
            warnings_json: JSON.stringify(dedupeWarnings(pipelineWarnings, ...finalVariants.map((variant) => variant.warnings))),
            error_message: null,
            processing_completed_at: new Date().toISOString(),
        });
    }
        ...
        await markFailed(env, processingLogId, String(error?.message || error || 'photo_3x4 pipeline failed'), {
            source_image_id: originalImageId,
            candidate_image_id: candidateImageId || undefined,
        });
    }
}
export async function handlePhoto3x4Queue(batch, env) {
...
export async function regeneratePhoto3x4Pipeline(env, logId, previewBaseOrigin) {
    const log = await fetchProcessingLog(env, logId);
    if (!log) {
        throw new Error('Processing log not found.');
    }
    const details = parseJsonField(log.processing_details, {});
...
export async function selectPhoto3x4Variant(env, logId, variantId) {
    const log = await fetchProcessingLog(env, logId);
    if (!log) {
        throw new Error('Processing log not found.');
    }
  ...
  `).bind(variantId, logId).first();
    ...
    return {
        ...
        warnings: parseJsonField(variant.warnings_json, []),
        selectedVariantId: variant.id,
    };
}
export async function getPhoto3x4ProcessingStatus(env, logId, previewBaseOrigin) {
    const log = await fetchProcessingLog(env, logId);
    if (!log) {
        return null;
    }
  ...
  `).bind(logId).all();
    ...
    return {
        ...
        warnings: parseJsonField(log.warnings_json, []),
        validationResult: parseJsonField(log.validation_result_json, null),
        variants: variants.results.map((variant) => ({
            ...
            label: PROMPT_PROFILE_LABELS[(variant.prompt_profile || 'balanced')] || 'Phuong an AI',
            validationResult: parseJsonField(variant.validation_result_json, null),
        })),
