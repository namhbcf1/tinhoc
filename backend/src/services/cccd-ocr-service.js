...
async function getImageBytes(env, imageId) {
    if (imageId.startsWith('cccd-uploads/')) {
        const object = await env.R2.get(imageId);
        if (!object)
            throw new Error('R2: ảnh không tồn tại - ' + imageId.slice(0, 60));
        return new Uint8Array(await object.arrayBuffer());
    }
    const signedUrl = await generateSignedImageURL(env, imageId, 2);
    const response = await fetch(signedUrl);
    if (!response.ok)
        throw new Error('CF Images fetch failed: ' + response.status);
    return new Uint8Array(await response.arrayBuffer());
}
async function buildOCRSpaceImageUrl(env, imageId) {
    if (!imageId) {
        return null;
    }
    if (imageId.startsWith('cccd-uploads/')) {
...
async function callOCRSpaceAttempt(input, attempt) {
    ...
    console.log(`[OCR] OCR.space raw engine=${attempt.engine} lang=${attempt.language} transport=${attempt.transport}:`, JSON.stringify(result).slice(0, 500));
    if (result.IsErroredOnProcessing || result.OCRExitCode !== 1) {
        throw new Error('OCR.space: ' + (result.ErrorMessage?.[0] || 'processing error'));
    }
    const text = result.ParsedResults?.[0]?.ParsedText || '';
    if (!text.trim())
        throw new Error('OCR.space: không phát hiện text');
    return text;
}
async function extractWithOCRSpace(input, type, attemptsLog) {
    const attempts = [
    ...
    for (const attempt of attempts) {
        try {
            ...
            console.warn(`[OCR] OCR.space attempt failed engine=${attempt.engine} lang=${attempt.language} transport=${attempt.transport || 'pending'}: ${message}`);
        }
    }
...
function lineHasAnyFragment(line, fragments) {
...
function looksLikeNonDataLine(line) {
    return lineHasAnyFragment(line, [
        'cong hoa',
        'socialist republic',
        'can cuoc',
...
function cleanPlaceValue(value) {
    return value
        .replace(/\b(?:place\s*of\s*ongin|place\s*of\s*origin|place\s*of\s*residence|quê\s*quán|quê\s*quán|que\s*quan|nơi\s*thường\s*trú|noi\s*thuong\s*tru)\b/giu, '')
        .replace(/[/:]+/g, ' ')
        .replace(/\s*,\s*/g, ', ')
        .replace(/\s+/g, ' ')
        .replace(/^(?:[,.\-\s])+/, '')
...
function normalizeOCRGenderValue(value) {
    const folded = foldOCRValue(value);
    if (!folded)
        return '';
    if (folded === 'nam' || folded === 'male' || folded === 'm')
        return 'Nam';
    if (folded === 'nu' || folded === 'female' || folded === 'f')
        return 'Nữ';
    if (folded.startsWith('nam'))
        return 'Nam';
    if (folded.startsWith('nu') || folded.includes('female'))
        return 'Nữ';
    return '';
}
function extractGenderAndNationalityFromLine(line) {
    const afterGenderLabel = line.replace(/.*?(?:giới\s*tính\s*\/\s*sex|gioi\s*tinh\s*\/\s*sex|giới\s*tính|gioi\s*tinh|sex|gender)\s*[:\-]?\s*/iu, '');
    const parts = afterGenderLabel.split(/(?:quốc\s*tịch\s*\/\s*nationality|quoc\s*tich\s*\/\s*nationality|quốc\s*tịch|quoc\s*tich|nationality)\s*[:\-]?\s*/iu);
...
function extractDateNearLabels(lines, labelFragments, minYear, maxYear) {
...
function extractDateByYearRange(text, minYear, maxYear) {
...
function extractLabeledMultilineValue(text, labelRegex, maxExtraLines = 1) {
...
function parseOCRTextToPrefill(text, type) {
    let cccd = '';
    let fullName = '';
    let dateOfBirth = '';
    let gender = '';
    ...
    let nationality = '';
    const lines = splitLines(text);
    const placeOfOrigin = extractLabeledMultilineValue(text, /(?:quê\s*quán\s*\/\s*place\s*of\s*origin|que\s*quan\s*\/\s*place\s*of\s*origin|place\s*of\s*origin|quê\s*quán|que\s*quan)/iu, 1);
    const placeOfResidence = extractLabeledMultilineValue(text, /(?:nơi\s*thường\s*trú\s*\/\s*place\s*of\s*residence|noi\s*thuong\s*tru\s*\/\s*place\s*of\s*residence|place\s*of\s*residence|place\s*of\s*residen|nơi\s*thường\s*trú|noi\s*thuong\s*tru|thường\s*trú|thuong\s*tru)/iu, 1);
    let issueDate = extractDateNearLabels(lines, ['ngay cap', 'date of issue', 'issued date', 'date month year', 'ngay thang nam'], 2015, 2035);
    ...
    if (type === 'cccd_front') {
        ...
        if (!fullName) {
            for (const line of lines) {
                if (/^\d/.test(line))
                    continue;
                if (/cccd|cmnd|căn cước|can cuoc|cộng hòa|cong hoa|việt nam|viet nam|socialist|republic|identity/iu.test(line))
                    continue;
                if (/ngày|ngay|quê|que|nơi|noi|giới|gioi|quốc|quoc|dân|dan|date|place|sex|nation/iu.test(line))
                    continue;
...
function hasUsefulExtraction(prefill) {
    return Boolean(prefill.cccd
        || prefill.fullName
        || prefill.dateOfBirth
        || prefill.gender
        || prefill.ethnicity
...
function normalizeCriticalValue(field, value) {
    if (!value)
        return '';
    if (field === 'cccd') {
...
function scoreCandidatePrefill(prefill, type) {
    ...
    if (type === 'cccd_front') {
        ...
        if (prefill.gender)
            score += 4;
        if (prefill.ethnicity)
            score += 2;
        if (prefill.placeOfOrigin)
            score += 2;
        if (prefill.placeOfResidence)
            score += 2;
    }
    else {
...
function detectCriticalConflicts(results, type) {
...
function mergeCandidatePrefills(results, type) {
    ...
    const fieldOrder = type === 'cccd_front'
...
export async function extractRegistrationPrefillFromImage(env, imageId, type) {
    const debug = {
        imageSize: 0,
...
export async function extractRegistrationPrefillFromCandidates(env, candidates, type) {
    const dedupedCandidates = candidates
        .filter((candidate, index, array) => (Boolean(candidate?.imageId)
