...
function pickFirstString(payload) {
...
function sanitizeJsonLikeText(value) {
    return quoteBareObjectKeys(value
        .replace(/[“”]/g, '"')
        .replace(/[‘’]/g, "'")
        .replace(/,\s*([}\]])/g, '$1'));
}
function tryParseJsonLikeText(text) {
...
export function parseCCCDExtractionPayload(payload) {
...
export function parseCCCDExtraction(text) {
    try {
