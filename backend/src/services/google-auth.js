...
// ── JWT generation ───────────────────────────────────────────────
async function generateJWT(clientEmail, privateKey, scopes, impersonateEmail) {
 ...
 */
export async function getGoogleAccessToken(env, scopes, impersonateEmail) {
    const jwt = await generateJWT(env.GOOGLE_CLIENT_EMAIL, env.GOOGLE_PRIVATE_KEY, scopes, impersonateEmail);
