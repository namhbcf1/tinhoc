...
export async function uploadDocument(c, form, file, user) {
    if (!form.title || !file)
        throw new Error('Thiếu title hoặc file');
    if (file.size > 1024 * 1024 * 1024)
        throw new Error('File quá lớn. Kích thước tối đa là 1GB');
    const ext = file.name.split('.').pop()?.toLowerCase();
    const mimeTypes = { pdf: 'application/pdf', doc: 'application/msword', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', xls: 'application/vnd.ms-excel', xlsx: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', mp4: 'video/mp4' };
    ...
    if (access_type === 'public') {
        ...
        if (class_ids) {
            ...
            for (const cid of clsIds) {
                ...
                if (!Number.isFinite(offlineClassId))
                    continue;
        ...
        if (form.cccd) {
            ...
            if (student)
                await DocRepo.createDocumentPermission(c.env.DB, { document_id: docId, permission_type: 'student', student_id: student.id });
        }
        if (form.student_ids) {
...
export async function shareDocumentIntoClasses(c, id, targets, user) {
...
export async function unshareDocument(c, id, type, targetId) {
    if (!['online_class', 'offline_class'].includes(type) || Number.isNaN(targetId))
        throw new Error('Target không hợp lệ');
    await DocRepo.unshareDocument(c.env.DB, id, type, targetId);
}
export async function getStudentDocuments(c, student_id, class_ids) {
    if (!student_id)
        throw new Error('Thiếu student_id');
    let student;
...
export async function getDocumentsByCCCD(c, cccd) {
    const student = await findStudentByCCCD(c.env.DB, cccd);
    if (!student)
        return [];
    const registrations = await getStudentRegistrations(c.env.DB, student.id);
    const classIds = Array.from(new Set((registrations || [])
        .filter((registration) => registration?.class_type !== 'thi')
        .map((registration) => Number.parseInt(String(registration.class_id), 10))
        .filter((classId) => Number.isFinite(classId))));
    return await DocRepo.getDocumentsForStudent(c.env.DB, student.id, classIds);
}
export async function processDocumentDownload(c, id, studentId) {
    const doc = await DocRepo.getDocumentById(c.env.DB, id);
    if (!doc)
        throw new Error('Tài liệu không tồn tại');
    if (studentId) {
    ...
    if (!object)
        throw new Error('File không tồn tại trong storage');
    return { object, doc };
}
export async function deleteDocument(c, id) {
    const doc = await DocRepo.getDocumentById(c.env.DB, id);
    if (!doc)
        throw new Error('Tài liệu không tồn tại');
    if (doc.r2_key)
        await c.env.R2.delete(doc.r2_key).catch(() => { });
