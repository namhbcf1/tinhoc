...
function stripKnownPrefixes(value, prefixes) {
...
function splitRawAddress(rawAddress) {
...
function buildNameVariants(name, kind) {
    const normalizedName = normalizeCompare(name);
    const stripped = kind === 'province'
...
function isTwoLevelDistrict(district) {
    if (!district)
        return false;
...
function dedupeDistrictIds(districts) {
    return [...new Set(districts.map((district) => district.DISTRICT_ID))];
}
function sanitizeAddressLine(parts) {
    return parts
        .map((part) => part.raw.replace(WHITESPACE_RE, ' ').trim())
        .filter(Boolean)
        .join(', ');
}
export async function resolveAddressAgainstCatalog(rawAddress, provinces, getDistricts, getWards) {
    const warnings = [];
    ...
    let wardMatch = null;
    let wardDistrict = districtMatch?.item || null;
    ...
    const leftoverParts = parts.filter((part) => part.raw !== provinceMatch.part.raw
