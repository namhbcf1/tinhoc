...
export async function getAllStudents(db, limit, offset) {
...
export async function countAllStudents(db) {
    const result = await db.prepare('SELECT COUNT(*) as count FROM students').first();
    return result?.count || 0;
}
export async function getStudentSummaryStats(db) {
...
export async function createStudent(db, data) {
  ...
  `).bind(data.cccd, data.ho, data.ten_dem || '', data.ten, data.ho_ten_full, data.ho_ten_normalized, data.ngay_sinh, data.noi_sinh, data.gioi_tinh, data.dan_toc || 'KINH', data.quoc_tich || 'VIỆT NAM', data.email, data.sdt, data.dia_chi, data.ngay_cap_cccd || null, data.don_vi_cong_tac || null, data.image_cccd_front || null, data.image_cccd_back || null, data.image_3x4 || null, data.cccd_front_image_id || null, data.cccd_back_image_id || null, data.photo_3x4_image_id || null).run();
    if (!result.success)
...
export async function getStudentEditHistory(db, studentId, limit, offset) {
    const result = await db.prepare(`
...
export async function logStudentEditHistory(db, data) {
    await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(data.student_id, data.admin_id, data.changed_by_type, data.field_name, data.old_value, data.new_value, data.ip_address, data.user_agent).run();
}
export async function getStudentRegistrations(db, studentId) {
  ...
  `).bind(studentId);
    ...
    const allRegistrations = [...(studyResult.results || []), ...(examResult.results || [])].sort((a, b) => {
