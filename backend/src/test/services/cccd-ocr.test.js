...
import { parseCCCDExtraction, parseCCCDExtractionPayload, } from '../../services/cccd-ocr-parser.js';
...
describe('extractRegistrationPrefillFromImage', () => {
    ...
    it('uses OCR.space engine 2 in Vietnamese when extraction succeeds', async () => {
        ...
        const result = await extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_front/test.jpg', 'cccd_front');
    ...
    it('retries OCR.space with engine 1 in Vietnamese if engine 2 fails', async () => {
        ...
        const result = await extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_back/test.jpg', 'cccd_back');
        expect(result.model).toBe('OCR.space');
        expect(result.prefill).toMatchObject({
            issueDate: '15/08/2021',
    ...
    it('returns timeout diagnostics when OCR.space hangs', async () => {
        ...
        const promise = extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_front/test.jpg', 'cccd_front');
    ...
    it('rejects oversized images before calling OCR.space', async () => {
        ...
        await expect(extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_front/test.jpg', 'cccd_front')).rejects.toThrow('Ảnh quá lớn');
    ...
    it('treats OCR text without recognizable fields as failure', async () => {
        ...
        await expect(extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_front/test.jpg', 'cccd_front')).rejects.toThrow('OCR đọc được text nhưng không nhận diện được trường CCCD nào');
        expect(fetchMock).toHaveBeenCalledTimes(2);
    });
    ...
    it('extracts female gender when sex and nationality share the same line', async () => {
        ...
        const result = await extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_front/test-female.jpg', 'cccd_front');
        expect(result.prefill).toMatchObject({
            cccd: '034303004393',
    ...
    it('extracts date of birth when OCR splits the label and date across lines', async () => {
        ...
        const result = await extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_front/test-split-dob.jpg', 'cccd_front');
        expect(result.prefill).toMatchObject({
            cccd: '034303004393',
            fullName: 'TRAN THI LINH',
            dateOfBirth: '08/09/2003',
    ...
    it('extracts issue date when OCR returns spaces instead of slashes', async () => {
        ...
        const result = await extractRegistrationPrefillFromImage(env, 'cccd-uploads/cccd_back/test-split-issue-date.jpg', 'cccd_back');
        expect(result.prefill).toMatchObject({
            issueDate: '27/06/2021',
        });
    });
