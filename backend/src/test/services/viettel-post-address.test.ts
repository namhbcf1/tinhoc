...
const districtsByProvince: Record<number, Array<{
  DISTRICT_ID: number;
...
const wardsByDistrict: Record<number, Array<{
  WARDS_ID: number;
  WARDS_NAME: string;
  DISTRICT_ID: number;
}>> = {
...
describe('viettel post address normalization', () => {
  it('resolves the sample Ca Mau two-level address', async () => {
    const result = await resolveAddressAgainstCatalog(
      'Ấp Tam Hưng, Xã Châu Thới, Tỉnh Cà Mau',
      provinces,
      async (provinceId) => districtsByProvince[provinceId] || [],
      async (districtId) => wardsByDistrict[districtId] || [],
    );
