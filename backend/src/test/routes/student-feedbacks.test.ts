...
async function makeStudentToken(studentId: number) {
  return generateJWT({
    id: studentId,
    role: 'student',
    type: 'student',
...
async function makeAdminToken(adminId: number) {
  return generateJWT({
    id: adminId,
    role: 'admin',
    type: 'admin',
...
async function seedStudent(id: number, name: string, cccd: string) {
  await env.DB.prepare('INSERT INTO students (id, cccd, ho_ten_full) VALUES (?, ?, ?)')
    .bind(id, cccd, name)
    .run();
}

async function seedAdmin(id: number, name: string) {
  await env.DB.prepare('INSERT INTO admins (id, full_name) VALUES (?, ?)')
    .bind(id, name)
    .run();
}

async function seedClass(id: number, className: string) {
...
async function seedEnrollment(classId: number, studentId: number, status = 'approved') {
...
describe('student feedback routes', () => {
  ...
  it('resubmits rejected feedback back to submitted on student update', async () => {
    ...
    const row = await env.DB.prepare('SELECT status, review_note_internal FROM student_feedbacks WHERE id = 1').first<any>();
