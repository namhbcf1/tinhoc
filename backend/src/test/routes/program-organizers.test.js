...
describe('program-organizers routes', () => {
    ...
    it('accepts admin JWT payloads that only expose role', async () => {
        ...
        const row = await env.DB.prepare('SELECT name, code, source_site, updated_by FROM program_organizers WHERE code = ?').bind('PTIT').first();
    ...
    it('returns a friendly duplicate-code error', async () => {
      ...
      `).bind('existing-organizer-uuid', 'PTIT', 'PTIT', 'Existing organizer', 1, 'edu').run();
        const response = await app.request('/program-organizers', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
