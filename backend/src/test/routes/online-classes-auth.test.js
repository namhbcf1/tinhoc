...
describe('Online Classes Router - Xác thực & Phân quyền', () => {
    ...
    describe('Authentication Middleware Rejections', () => {
        it('nên từ chối request không có token (401)', async () => {
            const res = await app.fetch(new Request('http://localhost/online-classes/1/available-students', {
                method: 'GET'
            }));
        ...
        it('nên từ chối token đã hết hạn (401)', async () => {
            ...
            const expiredToken = await generateJWT({ id: 1, role: 'admin', exp: Date.now() - 1000 }, JWT_SECRET);
            const res = await app.fetch(new Request('http://localhost/online-classes/1/available-students', {
        ...
        it('nên chặn student truy cập route admin (403)', async () => {
            ...
            const studentToken = await generateJWT({ id: 1, role: 'student', exp: Date.now() + 3600_000 }, JWT_SECRET);
            const res = await app.fetch(new Request('http://localhost/online-classes/1/available-students', {
        ...
        it('nên cho phép admin truy cập route admin (200 hoặc 404)', async () => {
            ...
            const adminToken = await generateJWT({ id: 99, role: 'admin', exp: Date.now() + 3600_000 }, JWT_SECRET);
