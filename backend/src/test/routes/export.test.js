...
async function seedTemplate(id, name, fileKey, dataStartRow = 10) {
...
async function seedOrganizer(uuid, name, code) {
    await env.DB.prepare(`
    INSERT INTO program_organizers (uuid, name, code)
    VALUES (?, ?, ?)
  `).bind(uuid, name, code).run();
}
async function seedExam(id, templateId, examName, examLevel = 'B1', organizerUuid = null) {
    await env.DB.prepare(`
...
async function seedStudentRegistration(examId, overrides = {}) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(overrides.ho ?? 'Nguyễn', overrides.ten_dem ?? 'Văn', overrides.ten ?? 'An', overrides.ho_ten_full ?? 'Nguyễn Văn An', overrides.ngay_sinh ?? '2002-09-05', overrides.noi_sinh ?? 'Hà Nội', overrides.gioi_tinh ?? 'Nam', overrides.dan_toc ?? 'Kinh', overrides.quoc_tich ?? 'Việt Nam', overrides.email ?? 'an@example.com', overrides.sdt ?? '0909123456', overrides.cccd ?? '012345678901', overrides.dia_chi ?? 'Hà Nội', overrides.ngay_cap_cccd ?? '2020-06-01', overrides.don_vi_cong_tac ?? 'ĐH ABC', overrides.image_3x4 ?? null, overrides.photo_3x4_image_id ?? null).run();
    ...
    it('exports exam-list from live approved list and ignores saved template file', async () => {
        ...
        const r2 = {
            get: vi.fn().mockResolvedValue({
                arrayBuffer: async () => createWorkbookBuffer([
