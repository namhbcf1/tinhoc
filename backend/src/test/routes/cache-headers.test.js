...
describe('default cache-control policy', () => {
    it('keeps public caching for anonymous public GET endpoints', () => {
        const policy = resolveDefaultCacheControl(makeRequest('https://vantrangedu-api.example.com/posts'), false);
    ...
    it('disables caching for authenticated student profile GET endpoints', () => {
        const policy = resolveDefaultCacheControl(makeRequest('https://vantrangedu-api.example.com/students/012345678901', {
            Authorization: 'Bearer token',
        }), false);
        expect(policy).toBe('private, no-store, max-age=0, must-revalidate');
    });
    it('does not override route-specific cache headers', () => {
        const policy = resolveDefaultCacheControl(makeRequest('https://vantrangedu-api.example.com/reports/summary'), true);
