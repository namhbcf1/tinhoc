...
async function makeAdminToken(id = 99) {
  return generateJWT(
    { id, role: 'admin', exp: Date.now() + 3_600_000 },
    JWT_SECRET
  );
}
...
async function insertTestClass(overrides = {}) {
  ...
  const result = await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, 'active', 50)
  `).bind(
    overrides.class_name ?? 'Lớp Test Thật',
    overrides.schedule_rule ?? 'WEEKLY:2',
    overrides.schedule_time ?? '19:00-21:00',
...
describe('GET /online-classes (getClassList)', () => {
  it('nên trả về 200 và danh sách rỗng khi không có lớp', async () => {
    const res = await app.fetch(
      new Request('http://localhost/online-classes')
    );
  ...
  it('nên ẩn meet_link với anonymous user', async () => {
    ...
    await env.DB.prepare(
      `UPDATE online_classes SET meet_link = 'https://meet.google.com/secret' WHERE class_name = ?`
    ).bind('Lớp Bí Mật').run();
...
describe('GET /online-classes/:id (getClassDetail)', () => {
  it('nên trả về 404 nếu không tìm thấy lớp học', async () => {
    const res = await app.fetch(
      new Request('http://localhost/online-classes/99999')
    );
  ...
  it('nên trả về 200 với thông tin chi tiết lớp học thật', async () => {
    ...
    const res = await app.fetch(
      new Request(`http://localhost/online-classes/${classId}`)
    );
...
describe('POST /online-classes (createClass)', () => {
  it('nên tạo lớp thành công với quyền admin (201)', async () => {
    ...
    const row = await env.DB.prepare(
      'SELECT id, class_name FROM online_classes WHERE class_name = ?'
    ).bind('Lớp Mới Tạo Thật').first();
...
describe('POST /online-classes/:id/enroll (enrollStudent)', () => {
  ...
  it('nên đăng ký thành công học viên với header X-Student-CCCD thật', async () => {
    ...
    const enrollment = await env.DB.prepare(
      `SELECT status FROM online_class_enrollments WHERE online_class_id = ?`
    ).bind(classId).first();
  ...
  it('nên trả về 400 khi lớp đã hết chỗ', async () => {
    ...
    await db.prepare(
      `INSERT INTO online_class_enrollments (online_class_id, student_id, status) VALUES (?, ?, 'active')`
...
describe('GET /online-classes/:id/available-students (admin only)', () => {
  it('nên trả về danh sách students chưa đăng ký', async () => {
    ...
    const res = await app.fetch(
      new Request(`http://localhost/online-classes/${classId}/available-students`, {
  ...
  it('nên trả về 404 nếu lớp không tồn tại', async () => {
    ...
    const res = await app.fetch(
      new Request('http://localhost/online-classes/99999/available-students', {
        headers: { Authorization: `Bearer ${adminToken}` }
      })
    );
