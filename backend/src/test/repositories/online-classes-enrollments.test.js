...
describe('online-classes repository - Enrollments (DB thật)', () => {
  ...
  it('createEnrollment - tạo enrollment pending thật trong DB', async () => {
    ...
    const row = await db.prepare(
      'SELECT id, status FROM online_class_enrollments WHERE online_class_id = ? AND student_id = ?'
  ...
  it('reEnroll - reset enrollment về pending (từ cancelled)', async () => {
    ...
    await db.prepare(
      "UPDATE online_class_enrollments SET status = 'cancelled' WHERE id = ?"
    ).bind(enrollmentId).run();

    await reEnroll(db, enrollmentId);

    const updated = await db.prepare(
      'SELECT status, rejection_reason FROM online_class_enrollments WHERE id = ?'
  ...
  it('rejectEnrollment - cập nhật status = rejected và lý do', async () => {
    ...
    const row = await db.prepare(
      'SELECT status, rejection_reason, approved_by FROM online_class_enrollments WHERE id = ?'
    ).bind(enrollmentId).first();
  ...
  it('approveEnrollment - cập nhật status = active và approved_by', async () => {
    ...
    const row = await db.prepare(
      'SELECT status, approved_by FROM online_class_enrollments WHERE id = ?'
    ).bind(enrollmentId).first();
  ...
  it('reactivateEnrollment - đặt lại status = active cho enrollment bị cancelled', async () => {
    ...
    const row = await db.prepare(
      'SELECT status FROM online_class_enrollments WHERE id = ?'
    ).bind(enrollmentId).first();
    expect(row.status).toBe('active');
  });
  ...
  it('cancelEnrollment - cập nhật status = cancelled cho đúng cặp classId + studentId', async () => {
    ...
    const row = await db.prepare(
      'SELECT status FROM online_class_enrollments WHERE online_class_id = ? AND student_id = ?'
    ).bind(classId, studentId).first();
