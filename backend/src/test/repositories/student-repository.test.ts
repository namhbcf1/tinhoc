...
async function seedStudent(db: D1Database, overrides: Record<string, string> = {}) {
  ...
  `).bind(
    cccd,
    overrides.ho ?? 'Nguyen',
    overrides.ten_dem ?? 'Van',
    overrides.ten ?? 'A',
    overrides.ho_ten_full ?? 'Nguyen Van A',
    overrides.ho_ten_normalized ?? 'nguyen van a',
    overrides.ngay_sinh ?? '2000-01-01',
    overrides.noi_sinh ?? 'Ha Noi',
    overrides.gioi_tinh ?? 'Nam',
    overrides.dan_toc ?? 'Kinh',
    overrides.quoc_tich ?? 'Viet Nam',
    overrides.email ?? 'a@example.com',
    overrides.sdt ?? '0900000000',
    overrides.dia_chi ?? 'Ha Noi'
  ).run();
}
