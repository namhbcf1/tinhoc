...
import {
  ...
  insertClass,
  updateClassMeetLink,
  updateClassCalendarInfo,
  updateClass,
  updateClassCalendarSync,
  deleteClass,
  findStudentByCccd,
  findStudentById,
  searchStudents
...
async function seedStudent(db, overrides = {}) {
  ...
  `).bind(
    ...
    overrides.ten ?? 'Test',
    overrides.ho_ten_full ?? 'Nguyễn Văn Test',
    overrides.ngay_sinh ?? '2000-01-01',
    overrides.noi_sinh ?? 'Hà Nội',
    overrides.gioi_tinh ?? 'Nam',
    overrides.email ?? 'test@example.com',
    overrides.sdt ?? '0900000001',
    overrides.dia_chi ?? '123 Đường Test',
  ).run();
...
describe('online-classes repository - Online Classes (DB thật)', () => {
  ...
  it('updateClassCalendarInfo cập nhật calendar_event_id và meet_link', async () => {
    ...
    const row = await env.DB.prepare(
      'SELECT calendar_event_id, meet_link FROM online_classes WHERE id = ?'
    ).bind(id).first();
  ...
  it('updateClassCalendarSync cập nhật tất cả sync fields', async () => {
    ...
    const row = await env.DB.prepare(
      'SELECT meet_link, calendar_event_id, recurrence FROM online_classes WHERE id = ?'
    ).bind(id).first();
...
describe('online-classes repository - Students (DB thật)', () => {
  ...
  it('searchStudents không keyword trả tối đa 50 bản ghi và không SELECT *', async () => {
    ...
    expectNoSelectStarQuery(
      'SELECT id, cccd, ho_ten_full, email, sdt, cccd_front_image_id, cccd_back_image_id, photo_3x4_image_id FROM students ORDER BY id DESC LIMIT 50'
    );
  });
  ...
  it('searchStudents với keyword lọc đúng theo tên và không SELECT *', async () => {
    ...
    expectNoSelectStarQuery(
      'SELECT id, cccd, ho_ten_full, email, sdt FROM students WHERE ho_ten_full LIKE ? LIMIT 50'
    );
  });
