...
export async function findTeacherByCode(db, teacher_code) {
...
export async function findTeacherById(db, id) {
  const result = await db.prepare(
...
export async function createTeacher(db, data) {
...
export async function updateTeacher(db, id, data) {
  const fields = [];
  const values = [];
...
export async function getAllTeachers(db, limit = 100, offset = 0) {
  const result = await db.prepare(
...
export async function searchTeachers(db, keyword) {
...
export async function getTeacherClasses(db, teacher_id) {
  // Get teacher info để match với teacher_name trong online_classes
  const teacher = await findTeacherById(db, teacher_id);
  if (!teacher) {
    return { results: [] };
  }

  // 1. Get classes from explicit assignment (class_teachers table)
  // This supports the Admin "Assign Teacher" feature
  ...
  // Query online classes - match bằng teacher_name (flexible matching)
  ...
  // Build query với LIKE để match flexible
  const onlineClassesQuery = teacherNameVariants.length > 0
    ...
    : `SELECT * FROM online_classes WHERE 1=0`; // No match if no teacher name
...
export async function getTeacherSchedule(db, teacher_id, week_start) {
  // week_start format: YYYY-MM-DD (ngày đầu tuần - Chủ nhật)
  ...
  // Query offline classes (class_schedules) - giờ đã xóa hết nên sẽ trống
  ...
  console.log(`[getTeacherSchedule] Found ${onlineClasses.results?.length || 0} online classes:`, onlineClasses.results?.map(oc => ({ id: oc.id, name: oc.class_name, teacher: oc.teacher_name })));

  // Convert online_classes thành format tương tự class_schedules
  const onlineSchedule = [];
  onlineClasses.results.forEach((oc) => {
    // Parse schedule_rule: WEEKLY:1,3,5 hoặc DAILY
    let days = [];
    ...
    // Tạo schedule item cho mỗi ngày trong tuần
...
export async function getTeacherExams(db, teacher_id) {
  const result = await db.prepare(
    `SELECT 
       es.id,
       es.class_id,
