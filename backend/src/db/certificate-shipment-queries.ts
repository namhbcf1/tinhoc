...
export async function getLatestShipmentByCertificate(db: D1Database, certificateId: number) {
  return db.prepare(`
    SELECT
      cs.*,
...
export async function getOpenShipmentByCertificate(db: D1Database, certificateId: number) {
...
export async function createCertificateShipment(db: D1Database, data: Record<string, any>) {
  ...
  `).bind(
    data.certificate_id,
    data.student_id,
    data.carrier || 'viettel_post',
    data.carrier_order_number || null,
    data.carrier_tracking_number || null,
    data.status || 'draft',
    data.receiver_name,
    data.receiver_phone,
    data.address_raw,
    ...
    data.ward_id ?? null,
    data.ward_name || null,
    data.normalized_full_address || null,
    data.resolution_status || 'unresolved',
    serializeJson(data.warnings_json ?? data.warnings ?? []),
    data.service_code || null,
    data.service_name || null,
    serializeJson(data.service_add_codes_json ?? data.service_add_codes ?? []),
    data.product_name || 'Chứng chỉ',
    data.product_description || 'Chứng chỉ, tài liệu',
    data.product_weight_grams || 250,
    data.declared_value || 0,
    data.shipping_fee ?? null,
    serializeJson(data.raw_request_json),
    serializeJson(data.raw_response_json),
...
export async function updateCertificateShipment(db: D1Database, shipmentId: number, data: Record<string, any>) {
  const updates: string[] = [];
  const values: any[] = [];

  const assign = (column: string, value: any, asJson = false) => {
