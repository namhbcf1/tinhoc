...
async function ensureClassExists(db, classId) {
    const row = await db
...
async function getSessionById(db, classId, sessionId) {
    const row = await db
        .prepare(`
        SELECT
          sessions.*,
...
export async function listClassSessions(db, classId) {
...
export async function createClassSession(db, classId, data) {
    await ensureClassExists(db, classId);
    ...
    const result = await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
      `)
        .bind(classId, sessionDate, startTime, endTime, sessionType, normalizeNullableText(data.title), normalizeNullableText(data.content_outline), parseNullableInteger(data.period_count), parseNullableInteger(data.teacher_id), normalizeNullableText(data.room), normalizeNullableText(data.meeting_link), normalizeNullableText(data.notes), parseNullableInteger(data.sort_order) ?? 0)
        .run();
    const sessionId = Number(result.meta.last_row_id);
    return getSessionById(db, classId, sessionId);
}
export async function updateClassSession(db, classId, sessionId, data) {
    await ensureClassExists(db, classId);
...
export async function deleteClassSession(db, classId, sessionId) {
    await ensureClassExists(db, classId);
    const existing = await getSessionById(db, classId, sessionId);
    if (!existing) {
        throw new Error('Không tìm thấy buổi học');
