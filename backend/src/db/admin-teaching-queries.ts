...
export async function getAdminClasses(db: D1Database, adminId: number) {
  const admin: any = await findAdminById(db, adminId);
  if (!admin) {
    return { results: [] };
  }
...
export async function getAdminSchedule(db: D1Database, adminId: number, week_start: string) {
  const admin: any = await findAdminById(db, adminId);
  if (!admin) {
    return { results: [] };
  }
...
export async function getAdminExams(db: D1Database, adminId: number) {
