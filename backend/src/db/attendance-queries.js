...
`;
const hasExamRegistrationPaymentStatusColumn = async (db) => {
    const result = await db.prepare(`PRAGMA table_info(exam_registrations)`).all();
    return (result.results || []).some((column) => column?.name === 'payment_status');
};
const buildExamRegistrationPaymentStatusSelect = (alias = 'r') => `
  ...
  END
`;
const getExamRegistrationPaymentStatusSelect = async (db, alias = 'r') => (await hasExamRegistrationPaymentStatusColumn(db)
    ? buildExamRegistrationPaymentStatusSelect(alias)
    : `'unknown'`);
const ensureExamRegistrationPaymentStatusColumn = async (db) => {
...
export async function getOnlineAttendanceByStudent(db, studentId) {
    const result = await db.prepare(`
    SELECT
 ...
 */
export async function getZoomCheckinsForExam(db, examScheduleId) {
    try {
        const result = await db.prepare(`
      SELECT
        oca.student_id,
...
// ========================================
export async function createExamSchedule(db, classId, examName, examDate, durationMinutes = 120, location = null, notes = null, templateId = null, metadata = {}) {
    const { zoom_link, zoom_link_backup, zoom_link_backup_2, zoom_link_backup_3, zoom_meeting_id, zoom_passcode, zoom_meeting_id_backup, zoom_passcode_backup, exam_type, exam_level, exam_category_id, exam_type_id, organizer_uuid, program_uuid, level_uuid, custom_field_payload, override_payload, updated_by, source_site, last_event_uuid, class_seed_name, class_seed_description, class_seed_schedule_rule, class_seed_schedule_time, class_seed_timezone, class_seed_start_date, class_seed_end_date, class_seed_teacher_name, class_seed_max_students, google_map_url, } = metadata;
    const result = await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(classId, examName, examDate, durationMinutes, location, google_map_url ?? null, notes, templateId, zoom_link ?? null, zoom_link_backup ?? null, zoom_link_backup_2 ?? null, zoom_link_backup_3 ?? null, zoom_meeting_id ?? null, zoom_passcode ?? null, zoom_meeting_id_backup ?? null, zoom_passcode_backup ?? null, exam_type ?? null, exam_level ?? null, exam_category_id ?? null, exam_type_id ?? null, organizer_uuid ?? null, program_uuid ?? null, level_uuid ?? null, custom_field_payload ?? null, override_payload ?? null, updated_by ?? null, source_site ?? 'edu', last_event_uuid ?? null, class_seed_name ?? null, class_seed_description ?? null, class_seed_schedule_rule ?? null, class_seed_schedule_time ?? null, class_seed_timezone ?? null, class_seed_start_date ?? null, class_seed_end_date ?? null, class_seed_teacher_name ?? null, class_seed_max_students ?? null).run();
    return result;
}
...
export async function updateExamSchedule(db, examId, data) {
    const { class_id, exam_name, exam_date, duration_minutes, location, notes, template_id, zoom_link, zoom_link_backup, zoom_link_backup_2, zoom_link_backup_3, zoom_meeting_id, zoom_passcode, zoom_meeting_id_backup, zoom_passcode_backup, exam_type, exam_level, exam_category_id, exam_type_id, organizer_uuid, program_uuid, level_uuid, custom_field_payload, override_payload, updated_by, source_site, last_event_uuid, class_seed_name, class_seed_description, class_seed_schedule_rule, class_seed_schedule_time, class_seed_timezone, class_seed_start_date, class_seed_end_date, class_seed_teacher_name, class_seed_max_students, google_map_url, } = data;
    const updates = [];
    const values = [];
    ...
    if (exam_level !== undefined) {
        ...
        values.push(exam_level ? exam_level.trim().toUpperCase() : null);
    }
...
export async function deleteExamSchedule(db, examId) {
    const result = await db.prepare('UPDATE exam_schedules SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL').bind(examId).run();
    return result;
}
// Khoi phuc tu thung rac
export async function restoreExamSchedule(db, examId) {
    const result = await db.prepare('UPDATE exam_schedules SET deleted_at = NULL WHERE id = ? AND deleted_at IS NOT NULL').bind(examId).run();
    return result;
}
...
export async function permanentlyDeleteExamSchedule(db, examId) {
    const result = await db.prepare('DELETE FROM exam_schedules WHERE id = ? AND deleted_at IS NOT NULL').bind(examId).run();
    return result;
}
...
function resolveExamRegistrationBucket(exam) {
    const combined = [
        exam?.exam_type,
        exam?.program_uuid,
        exam?.organizer_uuid,
...
function bucketConflicts(left, right) {
...
function getBucketMessageLabel(bucket) {
    if (bucket === 'english')
        return 'tiếng Anh (VSTEP/VEPT)';
    if (bucket === 'informatics')
        return 'tin học (PTIT...)';
...
export async function getStudentExams(db, studentId) {
  ...
  `).bind(studentId, studentId).all();
    ...
    const examBucketMap = new Map();
    for (const exam of exams) {
        examBucketMap.set(Number(exam.id), resolveExamRegistrationBucket(exam));
    }
    const activeRegistrations = exams.filter((exam) => ACTIVE_EXAM_REGISTRATION_STATUSES.has(exam.registration_status || '')
    ...
    return exams.map((exam) => {
        ...
        const shouldFlagConflict = !ACTIVE_EXAM_REGISTRATION_STATUSES.has(exam.registration_status || '')
            && isUpcomingExamRegistrationWindow(exam)
            && conflicts.length > 0;
        return {
            ...exam,
...
function getExamDateTimeKey(examDate) {
    if (!examDate) {
        return null;
    }
...
export function isUpcomingExamRegistrationWindow(exam) {
...
function getExamWindow(exam) {
    ...
    const durationMinutes = typeof exam.duration_minutes === 'number' && exam.duration_minutes > 0
...
export function examsOverlapInTime(leftExam, rightExam) {
...
export async function registerStudentForExam(db, examId, studentId, createdBy = null) {
    ...
    const adminId = typeof createdBy === 'number'
        ? createdBy
  ...
  `).bind(examId).first();
    if (!targetExam) {
        const err = new Error('EXAM_SCHEDULE_NOT_FOUND');
  ...
  `).bind(studentId, examId).all();
    const conflictingActives = isUpcomingExamRegistrationWindow(targetExam)
        ? (existingActives.results || []).filter((existingExam) => isUpcomingExamRegistrationWindow(existingExam)
...
export async function getExamRegistrationsForExport(db, examId, scope = 'approved') {
...
export async function updateExamRegistrationPaymentStatus(db, examId, studentId, paymentStatus) {
