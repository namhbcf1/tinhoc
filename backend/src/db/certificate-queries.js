...
export async function createCertificate(db, data) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(student_id, class_id, certificate_number, title, issued_date || new Date().toISOString().split('T')[0], pdf_url || null, issued_by || null).run();
    return result;
}
...
export async function getCertificateByNumber(db, certificateNumber) {
    const result = await db.prepare('SELECT * FROM certificates WHERE certificate_number = ?').bind(certificateNumber).first();
    return result;
}
