...
export async function createAdmin(db: D1Database, username: string, passwordHash: string, fullName: string, role = 'admin') {
  const result = await db.prepare(`
...
export async function updateAdminPassword(db: D1Database, id: number, passwordHash: string) {
  const result = await db.prepare(`
...
export async function createPasswordResetToken(db: D1Database, adminId: number, token: string, expiresAt: string) {
  const result = await db.prepare(`
...
export async function findPasswordResetToken(db: D1Database, token: string) {
  const result = await db.prepare(`
...
export async function markPasswordResetTokenAsUsed(db: D1Database, token: string) {
  const result = await db.prepare(`
...
export async function invalidateAllPasswordResetTokens(db: D1Database, adminId: number) {
  const result = await db.prepare(`
...
export async function createAuditLog(db: D1Database, adminId: number, action: string, tableName: string, recordId: number | string, oldValue: unknown, newValue: unknown) {
  await db.prepare(`
...
export async function logStudentEditHistory(db: D1Database, studentId: number, adminId: number, changedByType: string, fieldName: string, oldValue: unknown, newValue: unknown, ipAddress: string | null = null, userAgent: string | null = null) {
  const result = await db.prepare(`
...
export async function getStudentEditHistory(db: D1Database, studentId: number, limit = 100, offset = 0) {
  const result = await db.prepare(`
    SELECT
      seh.*,
