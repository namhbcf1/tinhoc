...
export async function getAllClasses(db: D1Database) {
...
export async function getOpenClasses(db: D1Database) {
  const result = await db.prepare(`
    SELECT *,
    class_type as loai,
    status as trang_thai,
...
export async function getClassById(db: D1Database, id: number) {
  const result = await db.prepare(`
    SELECT *,
    class_type as loai,
    status as trang_thai,
...
export async function createClass(db: D1Database, data: Record<string, any>) {
  const {
...
export async function updateClass(db: D1Database, id: number, data: Record<string, unknown>) {
  const {
    ten_lop, ma_lop, ngay_thi, ngay_bat_dau, ngay_ket_thuc,
...
export async function deleteClass(db: D1Database, id: number) {
