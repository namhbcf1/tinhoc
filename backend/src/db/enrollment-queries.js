...
// FIND
// ========================================
export async function findRegistration(db, studentId, classId) {
    const result = await db.prepare('SELECT * FROM registrations WHERE student_id = ? AND class_id = ?').bind(studentId, classId).first();
    return result;
}
export async function getRegistrationsByClass(db, classId) {
...
export async function getStudentRegistrations(db, studentId) {
  ...
  `).bind(studentId);
    ...
    return [...studyRegistrations, ...examRegistrations].sort((a, b) => {
...
// CREATE
// ========================================
export async function createRegistration(db, studentId, classId) {
...
// UPDATE
// ========================================
export async function updateRegistrationStatus(db, id, status, paymentStatus) {
...
export async function updateSoPhach(db, registrationId, soPhach) {
    const result = await db.prepare(`
    UPDATE registrations SET
      so_phach = ?,
      updated_at = datetime('now', '+7 hours')
...
// DELETE
// ========================================
export async function deleteRegistration(db, registrationId) {
    // Get class_id before deleting
    const registration = await db.prepare('SELECT class_id FROM registrations WHERE id = ?').bind(registrationId).first();
    if (!registration) {
        return { success: false, error: 'Registration not found' };
    }
    // Delete registration
    const result = await db.prepare('DELETE FROM registrations WHERE id = ?').bind(registrationId).run();
...
// HELPERS
// ========================================
export async function syncClassStudentCount(db, classId) {
