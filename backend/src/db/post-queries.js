...
export async function createPost(db, postData) {
    const { title, slug, content, excerpt, category, tags, featured_image, video_url, author_id, status = 'draft', publish_at } = postData;
  ...
  `).bind(title, postSlug, content, excerpt || null, category || 'general', tags || null, featured_image || null, video_url || null, author_id || 0, status, publish_at || null).run();
    if (!result.success) {
...
export async function updatePost(db, postId, postData) {
    const { title, slug, content, excerpt, category, tags, featured_image, video_url, status, publish_at } = postData;
