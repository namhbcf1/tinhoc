...
export async function createClass(db, data) {
    const { ten_lop, ma_lop, ngay_thi, ngay_bat_dau, ngay_ket_thuc, gio_thi, dia_diem, hoc_phi, open_at, close_at, status, class_type, max_students } = data;
    try {
        // Dam bao ngay_thi luon co gia tri (cot NOT NULL trong DB)
      ...
      VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(ten_lop || null, ma_lop || null, finalNgayThi, ngay_bat_dau || null, ngay_ket_thuc || null, gio_thi || null, dia_diem || null, hoc_phi || 0, open_at || null, close_at || null, status || 'open', class_type || 'hoc', max_students || null).run();
        return result;
    }
    catch (error) {
...
export async function updateClass(db, id, data) {
    const { ten_lop, ma_lop, ngay_thi, ngay_bat_dau, ngay_ket_thuc, gio_thi, dia_diem, hoc_phi, open_at, close_at, status, class_type, max_students, current_students } = data;
    const updates = [];
    const values = [];
    if (ten_lop !== undefined) {
        updates.push('ten_lop = ?');
        values.push(ten_lop);
    }
    if (ma_lop !== undefined) {
        updates.push('ma_lop = ?');
    ...
    if (ngay_thi !== undefined) {
        updates.push('ngay_thi = ?');
        values.push(ngay_thi);
    }
    if (ngay_bat_dau !== undefined) {
        updates.push('ngay_bat_dau = ?');
        values.push(ngay_bat_dau);
    }
    if (ngay_ket_thuc !== undefined) {
        updates.push('ngay_ket_thuc = ?');
        values.push(ngay_ket_thuc);
    }
    if (gio_thi !== undefined) {
        updates.push('gio_thi = ?');
        values.push(gio_thi);
    }
    if (dia_diem !== undefined) {
        updates.push('dia_diem = ?');
        values.push(dia_diem);
    }
    if (hoc_phi !== undefined) {
        updates.push('hoc_phi = ?');
        values.push(hoc_phi);
    }
    if (open_at !== undefined) {
        updates.push('open_at = ?');
        values.push(open_at);
    }
    if (close_at !== undefined) {
        updates.push('close_at = ?');
