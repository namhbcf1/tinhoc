...
export async function findStudentByCCCD(db, cccd) {
    const result = await db.prepare('SELECT * FROM students WHERE cccd = ?').bind(cccd).first();
    return result;
}
export async function findStudentByEmailOrPhone(db, email, sdt) {
    const result = await db.prepare('SELECT * FROM students WHERE email = ? OR sdt = ?').bind(email, sdt).all();
...
export async function createStudent(db, data) {
    const { cccd, ho, ten_dem, ten, ho_ten_full, ho_ten_normalized, ngay_sinh, noi_sinh, gioi_tinh, dan_toc, quoc_tich, email, sdt, dia_chi, ngay_cap_cccd, don_vi_cong_tac, image_cccd_front, image_cccd_back, image_3x4, cccd_front_image_id, cccd_back_image_id, photo_3x4_image_id } = data;
  ...
  `).bind(cccd, ho, ten_dem, ten, ho_ten_full, ho_ten_normalized, ngay_sinh, noi_sinh, gioi_tinh, dan_toc || 'KINH', quoc_tich || 'VIỆT NAM', email, sdt, dia_chi, ngay_cap_cccd || null, don_vi_cong_tac || null, image_cccd_front || null, image_cccd_back || null, image_3x4 || null, cccd_front_image_id || null, cccd_back_image_id || null, photo_3x4_image_id || null).run();
    return result;
}
// ========================================
// UPDATE
...
export async function searchStudents(db, keyword) {
    ...
    LIMIT 100
  `;
    const result = await db.prepare(query).bind(searchPattern, `%${searchTerm.toLowerCase()}%`, searchPattern, searchPattern, searchPattern, exactPattern, exactPattern, searchTerm, `${normalized}%`, `${normalized}%`, `${normalized}%`, exactPattern).all();
    ...
    results.sort((a, b) => {
        ...
        if (a.cccd === searchTerm && b.cccd !== searchTerm)
            return -1;
        if (b.cccd === searchTerm && a.cccd !== searchTerm)
            return 1;
        if (aName.startsWith(normalized) && !bName.startsWith(normalized))
            return -1;
        if (bName.startsWith(normalized) && !aName.startsWith(normalized))
            return 1;
        if (aPhone.includes(normalizedPhone) && !bPhone.includes(normalizedPhone))
            return -1;
        if (bPhone.includes(normalizedPhone) && !aPhone.includes(normalizedPhone))
            return 1;
