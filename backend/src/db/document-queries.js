...
export async function createDocument(db, data) {
    const { title, description, file_name, file_size, file_type, r2_key, uploaded_by, folder_id, visibility, doc_type, valid_from, valid_until, organizer_uuid, program_uuid, level_uuid, custom_field_payload, override_payload } = data;
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?, ?, ?, ?, 'edu')
    `).bind(title, description || null, r2_key, // stored in file_url column
        file_name, file_size, file_type, valid_until || null, uploaded_by || null, folder_id || null, visibility || 'internal', organizer_uuid || null, program_uuid || null, level_uuid || null, custom_field_payload || null, override_payload || null).run();
        return {
            success: true,
            meta: result.meta,
        };
...
export async function createDocumentPermission(db, data) {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(document_id, permission_type, class_id || null, online_class_id || null, student_id || null).run();
        return {
            success: true,
            meta: result.meta,
        };
...
export async function getDocumentById(db, id) {
    const result = await db.prepare(`SELECT * FROM documents WHERE id = ? AND source_site IN ('edu', 'system')`).bind(id).first();
    return result;
}
export async function getDocumentPermissions(db, documentId) {
    const result = await db.prepare(`
...
export async function getDocumentsByCCCD(db, cccd) {
    // Get student ID from CCCD
    const student = await db.prepare('SELECT id FROM students WHERE cccd = ?').bind(cccd).first();
    if (!student)
        return [];
    return getDocumentsForStudent(db, student.id, cccd, []);
}
export async function getDocumentsForStudent(db, studentId, studentCCCD, classIds = []) {
...
export async function getAllDocuments(db, limit = 100, offset = 0) {
    const result = await db.prepare(`
    SELECT d.*,
...
export async function deleteDocumentPermissions(db, documentId) {
    const result = await db.prepare('DELETE FROM document_permissions WHERE document_id = ?').bind(documentId).run();
    return result;
}
export async function recordDocumentDownload(db, data) {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?)
    `).bind(document_id, student_id, ip_address || null, user_agent || null).run();
        return result;
    }
    catch (e) {
      ...
      VALUES (?, ?)
    `).bind(document_id, student_id).run();
        return result;
    }
}
export async function getDocumentDownloadStats(db, documentId) {
    const result = await db.prepare(`
