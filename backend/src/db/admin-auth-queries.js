...
export async function findAdminById(db, id) {
    const result = await db.prepare('SELECT * FROM admins WHERE id = ?').bind(id).first();
    return result;
}
...
export async function updateAdminLastLogin(db, id) {
    await db.prepare("UPDATE admins SET last_login = datetime('now', '+7 hours') WHERE id = ?").bind(id).run();
}
// ========================================
...
export async function createAuditLog(db, adminId, action, tableName, recordId, oldValue, newValue) {
    ...
    VALUES(?, ?, ?, ?, ?, ?)
    `).bind(adminId, action, tableName, recordId, oldValue ? JSON.stringify(oldValue) : null, newValue ? JSON.stringify(newValue) : null).run();
}
// ========================================
...
export async function logStudentEditHistory(db, studentId, adminId, changedByType, fieldName, oldValue, newValue, ipAddress = null, userAgent = null) {
  ...
  `).bind(studentId, adminId, changedByType, fieldName, oldValue !== null && oldValue !== undefined ? String(oldValue) : null, newValue !== null && newValue !== undefined ? String(newValue) : null, ipAddress, userAgent).run();
    return result;
}
