...
export function resolveDefaultCacheControl(request, hasExistingCacheControl = false) {
    if (request.method !== 'GET' || hasExistingCacheControl) {
        return null;
    }
    const path = new URL(request.url).pathname;
    const hasAuthContext = Boolean(request.headers.get('Authorization') || request.headers.get('X-Student-CCCD'));
...
function isReadOnlyModeEnabled(value) {
    if (!value)
        return false;
...
const worker = {
    fetch: app.fetch,
    queue(batch, env) {
        return handlePhoto3x4Queue(batch, env);
    },
};
