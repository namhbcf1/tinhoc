...
export function hasProgramPlatformOperatorAccess(user: JWTPayload | undefined | null) {
  const role = user?.role || user?.type || user?.user_type || null;
  return role === 'admin' || role === 'super_admin';
}

export function getProgramPlatformErrorMessage(error: unknown, fallback: string) {
  ...
  if (
    /UNIQUE constraint failed: programs\.organizer_uuid, programs\.code/i.test(message) ||
  ...
  if (
    /UNIQUE constraint failed: program_levels\.program_uuid, program_levels\.code/i.test(message) ||
  ...
  if (
    /UNIQUE constraint failed: field_options\.field_definition_uuid, field_options\.value/i.test(message) ||
