export const PROGRAM_DELIVERY_MODES = ['internal_training', 'external_redirect'];
...
export const PROGRAM_SCHEDULE_MODELS = ['session_based', 'weekly_template'];
...
function parseEnumValue(value, allowedValues, fallback, errorLabel) {
    if (value == null || value === '') {
        return fallback;
    }
    const normalized = String(value).trim();
...
function supportsLegacyExamMapping(assessmentMode) {
...
function mapProgramRow(row, organizer, options = {}) {
    return {
        id: row.id,
        uuid: row.uuid,
...
function mapLevelRow(row, program) {
    return {
        id: row.id,
        uuid: row.uuid,
...
function mapFieldDefinitionRow(row) {
    return {
        id: row.id,
        uuid: row.uuid,
...
function mapFieldOptionRow(row) {
    return {
        id: row.id,
        uuid: row.uuid,
...
async function resolveLegacyExamCategoryId(db, code, name) {
    try {
        const row = await db
            .prepare(`
          SELECT id
...
async function resolveLegacyExamTypeId(db, code, name) {
    try {
        const row = await db
            .prepare(`
          SELECT id
...
const PROGRAM_PLATFORM_SEED = {
    ...
    programs: [
        { uuid: '20000000-0000-4000-8000-000000000001', organizerCode: 'EDUGLOBAL', code: 'VSTEP', name: 'VSTEP', deliveryMode: 'internal_training', trainingEnabled: 1, linkedClassEnabled: 1, visibleOnEduPublic: 1, visibleOnEduAdmin: 1, visibleOnExamTeacher: 1, visibleOnExamStudent: 1, redirectUrl: null, assessmentMode: 'official_exam', certificateEnabled: 1, scheduleModel: 'session_based' },
        { uuid: '20000000-0000-4000-8000-000000000002', organizerCode: 'EDUGLOBAL', code: 'VEPT', name: 'VEPT', deliveryMode: 'external_redirect', trainingEnabled: 0, linkedClassEnabled: 0, visibleOnEduPublic: 1, visibleOnEduAdmin: 1, visibleOnExamTeacher: 0, visibleOnExamStudent: 0, redirectUrl: '/vept', assessmentMode: 'official_exam', certificateEnabled: 1, scheduleModel: 'session_based' },
        { uuid: '20000000-0000-4000-8000-000000000003', organizerCode: 'EDUGLOBAL', code: 'TIN_HOC', name: 'Tin học', deliveryMode: 'internal_training', trainingEnabled: 1, linkedClassEnabled: 1, visibleOnEduPublic: 1, visibleOnEduAdmin: 1, visibleOnExamTeacher: 1, visibleOnExamStudent: 1, redirectUrl: null, assessmentMode: 'mixed', certificateEnabled: 1, scheduleModel: 'session_based' },
        { uuid: '20000000-0000-4000-8000-000000000004', organizerCode: 'HVKHQS', code: 'VSTEP', name: 'VSTEP', deliveryMode: 'internal_training', trainingEnabled: 1, linkedClassEnabled: 1, visibleOnEduPublic: 1, visibleOnEduAdmin: 1, visibleOnExamTeacher: 1, visibleOnExamStudent: 1, redirectUrl: null, assessmentMode: 'official_exam', certificateEnabled: 1, scheduleModel: 'session_based' },
        { uuid: '20000000-0000-4000-8000-000000000005', organizerCode: 'HVKHQS', code: 'VEPT', name: 'VEPT', deliveryMode: 'external_redirect', trainingEnabled: 0, linkedClassEnabled: 0, visibleOnEduPublic: 1, visibleOnEduAdmin: 1, visibleOnExamTeacher: 0, visibleOnExamStudent: 0, redirectUrl: '/vept', assessmentMode: 'official_exam', certificateEnabled: 1, scheduleModel: 'session_based' },
        { uuid: '20000000-0000-4000-8000-000000000006', organizerCode: 'HVKHQS', code: 'TIN_HOC', name: 'Tin học', deliveryMode: 'internal_training', trainingEnabled: 1, linkedClassEnabled: 1, visibleOnEduPublic: 1, visibleOnEduAdmin: 1, visibleOnExamTeacher: 1, visibleOnExamStudent: 1, redirectUrl: null, assessmentMode: 'mixed', certificateEnabled: 1, scheduleModel: 'session_based' },
    ],
    levels: [
...
export async function recordProgramPlatformSyncEvent(db, input) {
    ...
    await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `)
        .bind(eventUuid, input.entityType, input.entityUuid, input.action, input.sourceSite, changedAt, payloadJson)
        .run();
    return eventUuid;
}
export async function ensureSeedProgramPlatform(db) {
    ...
    for (const program of PROGRAM_PLATFORM_SEED.programs) {
        ...
        await db
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'edu')
        `)
            .bind(program.uuid, organizer.uuid, program.name, program.code, program.deliveryMode, program.trainingEnabled, program.linkedClassEnabled, program.visibleOnEduPublic, program.visibleOnEduAdmin, program.visibleOnExamTeacher, program.visibleOnExamStudent, program.redirectUrl, legacyCategoryId, legacyTypeId, program.assessmentMode, program.certificateEnabled, program.scheduleModel)
            .run();
    }
    for (const level of PROGRAM_PLATFORM_SEED.levels) {
        const program = PROGRAM_PLATFORM_SEED.programs.find((item) => item.organizerCode === level.organizerCode && item.code === level.programCode);
        if (!program) {
            continue;
        }
        await db
...
export async function listProgramOrganizers(db, options = {}) {
    ...
    const result = await db
      ...
      `)
        ...
        .all();
    return (result.results || []).map((row) => mapOrganizerRow(row));
}
export async function getProgramOrganizerByUuid(db, uuid) {
    const row = await db
...
export async function createProgramOrganizer(db, payload, actor, sourceSite) {
    ...
    await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .bind(uuid, name, code, normalizeString(payload.description), parseBooleanFlag(payload.is_active, true), updatedBy, sourceSite, eventUuid, nowIso())
        .run();
...
export async function updateProgramOrganizer(db, uuid, payload, actor, sourceSite) {
    ...
    await db
        ...
        WHERE uuid = ?
      `)
        .bind(name, code, normalizeString(payload.description) ?? existing.description, payload.is_active === undefined ? existing.is_active : parseBooleanFlag(payload.is_active, true), updatedBy, sourceSite, eventUuid, nowIso(), uuid)
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
        entityType: 'program_organizer',
...
export async function listPrograms(db, options = {}) {
    await ensureSeedProgramPlatform(db);
    const result = await db
        .prepare(`
        SELECT p.*,
      ...
      `)
        ...
        .all();
    return (result.results || []).map((row) => mapProgramRow(row, {
...
export async function getProgramByUuid(db, uuid) {
    const row = await db.prepare(`SELECT * FROM programs WHERE uuid = ? LIMIT 1`).bind(uuid).first();
    return row || null;
}
export async function createProgram(db, payload, actor, sourceSite) {
    ...
    const deliveryMode = normalizeString(payload.delivery_mode);
    if (!deliveryMode || !PROGRAM_DELIVERY_MODES.includes(deliveryMode)) {
        throw new Error('Invalid delivery_mode');
    }
    const assessmentMode = parseEnumValue(payload.assessment_mode, PROGRAM_ASSESSMENT_MODES, 'official_exam', 'assessment_mode');
    const scheduleModel = parseEnumValue(payload.schedule_model, PROGRAM_SCHEDULE_MODELS, 'session_based', 'schedule_model');
    const uuid = normalizeString(payload.uuid) || createUuid();
    const eventUuid = normalizeString(payload.last_event_uuid) || createUuid();
    const updatedBy = parseOptionalInteger(payload.updated_by) ?? resolveActorId(actor);
    ...
    await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .bind(uuid, organizerUuid, name, code, normalizeString(payload.description), deliveryMode, parseBooleanFlag(payload.training_enabled, true), parseBooleanFlag(payload.linked_class_enabled, deliveryMode === 'internal_training'), parseBooleanFlag(payload.visible_on_edu_public, true), parseBooleanFlag(payload.visible_on_edu_admin, true), parseBooleanFlag(payload.visible_on_exam_teacher, deliveryMode === 'internal_training'), parseBooleanFlag(payload.visible_on_exam_student, deliveryMode === 'internal_training'), normalizeString(payload.redirect_url), parseBooleanFlag(payload.is_active, true), legacyCategoryId, legacyTypeId, assessmentMode, parseBooleanFlag(payload.certificate_enabled, false), scheduleModel, updatedBy, sourceSite, eventUuid, nowIso())
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
        entityType: 'program',
...
export async function updateProgram(db, uuid, payload, actor, sourceSite) {
    ...
    const name = normalizeString(payload.name) || existing.name;
    const code = normalizeCode(payload.code || existing.code) || existing.code;
    const deliveryMode = (normalizeString(payload.delivery_mode) || existing.delivery_mode);
    if (!PROGRAM_DELIVERY_MODES.includes(deliveryMode)) {
        throw new Error('Invalid delivery_mode');
    }
    const assessmentMode = parseEnumValue(payload.assessment_mode, PROGRAM_ASSESSMENT_MODES, existing.assessment_mode, 'assessment_mode');
    const scheduleModel = parseEnumValue(payload.schedule_model, PROGRAM_SCHEDULE_MODELS, existing.schedule_model, 'schedule_model');
    const eventUuid = normalizeString(payload.last_event_uuid) || createUuid();
    const updatedBy = parseOptionalInteger(payload.updated_by) ?? resolveActorId(actor);
    const legacyCategoryId = supportsLegacyExamMapping(assessmentMode)
    ...
    await db
        ...
        WHERE uuid = ?
      `)
        .bind(organizerUuid, name, code, normalizeString(payload.description) ?? existing.description, deliveryMode, payload.training_enabled === undefined ? existing.training_enabled : parseBooleanFlag(payload.training_enabled, true), payload.linked_class_enabled === undefined ? existing.linked_class_enabled : parseBooleanFlag(payload.linked_class_enabled, deliveryMode === 'internal_training'), payload.visible_on_edu_public === undefined ? existing.visible_on_edu_public : parseBooleanFlag(payload.visible_on_edu_public, true), payload.visible_on_edu_admin === undefined ? existing.visible_on_edu_admin : parseBooleanFlag(payload.visible_on_edu_admin, true), payload.visible_on_exam_teacher === undefined ? existing.visible_on_exam_teacher : parseBooleanFlag(payload.visible_on_exam_teacher, deliveryMode === 'internal_training'), payload.visible_on_exam_student === undefined ? existing.visible_on_exam_student : parseBooleanFlag(payload.visible_on_exam_student, deliveryMode === 'internal_training'), normalizeString(payload.redirect_url) ?? existing.redirect_url, payload.is_active === undefined ? existing.is_active : parseBooleanFlag(payload.is_active, true), legacyCategoryId, legacyTypeId, assessmentMode, payload.certificate_enabled === undefined ? existing.certificate_enabled : parseBooleanFlag(payload.certificate_enabled, false), scheduleModel, updatedBy, sourceSite, eventUuid, nowIso(), uuid)
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
        entityType: 'program',
...
export async function listProgramLevels(db, options = {}) {
    await ensureSeedProgramPlatform(db);
    const result = await db
        .prepare(`
      ...
      `)
        ...
        .all();
    return (result.results || []).map((row) => mapLevelRow(row, {
...
export async function getProgramLevelByUuid(db, uuid) {
    const row = await db.prepare(`SELECT * FROM program_levels WHERE uuid = ? LIMIT 1`).bind(uuid).first();
    return row || null;
}
export async function createProgramLevel(db, payload, actor, sourceSite) {
    ...
    await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .bind(uuid, programUuid, name, code, normalizeString(payload.description), parseOptionalInteger(payload.sort_order) ?? 0, parseBooleanFlag(payload.is_active, true), updatedBy, sourceSite, eventUuid, nowIso())
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
...
export async function updateProgramLevel(db, uuid, payload, actor, sourceSite) {
    ...
    await db
        ...
        WHERE uuid = ?
      `)
        .bind(programUuid, name, code, normalizeString(payload.description) ?? existing.description, parseOptionalInteger(payload.sort_order) ?? existing.sort_order, payload.is_active === undefined ? existing.is_active : parseBooleanFlag(payload.is_active, true), updatedBy, sourceSite, eventUuid, nowIso(), uuid)
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
        entityType: 'program_level',
...
export async function listFieldDefinitions(db, options = {}) {
    const result = await db
        .prepare(`
        SELECT *
      ...
      `)
        .bind(options.targetEntityType || null, options.targetEntityType || null, options.ownerEntityType || null, options.ownerEntityType || null, options.ownerEntityUuid || null, options.ownerEntityUuid || null, options.includeInactive ? 1 : 0)
        .all();
    return (result.results || []).map((row) => mapFieldDefinitionRow(row));
}
export async function getFieldDefinitionByUuid(db, uuid) {
    const row = await db.prepare(`SELECT * FROM field_definitions WHERE uuid = ? LIMIT 1`).bind(uuid).first();
    return row || null;
}
export async function createFieldDefinition(db, payload, actor, sourceSite) {
    ...
    if (!label || !fieldKey || !fieldType || !FIELD_TYPES.includes(fieldType) || !targetEntityType) {
    ...
    await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .bind(uuid, fieldKey, label, normalizeString(payload.description), fieldType, targetEntityType, ownerEntityType, ownerEntityUuid, normalizeString(payload.placeholder), normalizeString(payload.help_text || payload.helper_text), parseJsonValue(payload.config ?? payload.config_json), parseBooleanFlag(payload.searchable, false), parseBooleanFlag(payload.filterable, false), parseBooleanFlag(payload.exportable, false), parseBooleanFlag(payload.reportable, false), parseBooleanFlag(payload.visible_on_edu_public, false), parseBooleanFlag(payload.visible_on_edu_admin, true), parseBooleanFlag(payload.visible_on_exam_teacher, true), parseBooleanFlag(payload.visible_on_exam_student, false), parseBooleanFlag(payload.is_active, true), parseOptionalInteger(payload.sort_order) ?? 0, updatedBy, sourceSite, eventUuid, nowIso())
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
...
export async function updateFieldDefinition(db, uuid, payload, actor, sourceSite) {
    ...
    await db
        ...
        WHERE uuid = ?
      `)
        .bind(fieldKey, label, normalizeString(payload.description) ?? existing.description, fieldType, targetEntityType, ownerEntityType, ownerEntityUuid, normalizeString(payload.placeholder) ?? existing.placeholder, normalizeString(payload.help_text || payload.helper_text) ?? existing.help_text, payload.config === undefined && payload.config_json === undefined
        ? existing.config_json
        : parseJsonValue(payload.config ?? payload.config_json), payload.searchable === undefined ? existing.searchable : parseBooleanFlag(payload.searchable, false), payload.filterable === undefined ? existing.filterable : parseBooleanFlag(payload.filterable, false), payload.exportable === undefined ? existing.exportable : parseBooleanFlag(payload.exportable, false), payload.reportable === undefined ? existing.reportable : parseBooleanFlag(payload.reportable, false), payload.visible_on_edu_public === undefined ? existing.visible_on_edu_public : parseBooleanFlag(payload.visible_on_edu_public, false), payload.visible_on_edu_admin === undefined ? existing.visible_on_edu_admin : parseBooleanFlag(payload.visible_on_edu_admin, true), payload.visible_on_exam_teacher === undefined ? existing.visible_on_exam_teacher : parseBooleanFlag(payload.visible_on_exam_teacher, true), payload.visible_on_exam_student === undefined ? existing.visible_on_exam_student : parseBooleanFlag(payload.visible_on_exam_student, false), payload.is_active === undefined ? existing.is_active : parseBooleanFlag(payload.is_active, true), parseOptionalInteger(payload.sort_order) ?? existing.sort_order, updatedBy, sourceSite, eventUuid, nowIso(), uuid)
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
        entityType: 'field_definition',
...
export async function listFieldOptions(db, options = {}) {
    const result = await db
        .prepare(`
        SELECT *
        FROM field_options
      ...
      `)
        ...
        .all();
    return (result.results || []).map((row) => mapFieldOptionRow(row));
}
export async function getFieldOptionByUuid(db, uuid) {
    const row = await db.prepare(`SELECT * FROM field_options WHERE uuid = ? LIMIT 1`).bind(uuid).first();
    return row || null;
}
export async function createFieldOption(db, payload, actor, sourceSite) {
    ...
    await db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .bind(uuid, fieldDefinitionUuid, label, value, normalizeString(payload.color), parseOptionalInteger(payload.sort_order) ?? 0, parseBooleanFlag(payload.is_active, true), updatedBy, sourceSite, eventUuid, nowIso())
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
...
export async function updateFieldOption(db, uuid, payload, actor, sourceSite) {
    ...
    await db
        ...
        WHERE uuid = ?
      `)
        .bind(fieldDefinitionUuid, label, value, normalizeString(payload.color) ?? existing.color, parseOptionalInteger(payload.sort_order) ?? existing.sort_order, payload.is_active === undefined ? existing.is_active : parseBooleanFlag(payload.is_active, true), updatedBy, sourceSite, eventUuid, nowIso(), uuid)
        .run();
    await recordProgramPlatformSyncEvent(db, {
        eventUuid,
        entityType: 'field_option',
...
export async function resolveProgramContext(db, input) {
