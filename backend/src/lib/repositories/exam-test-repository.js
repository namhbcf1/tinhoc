...
export async function createExamTest(db, data) {
    const { exam_type_id, level, title, description, duration_minutes, passing_score, shuffle_questions, shuffle_options, created_by, requires_registration, max_attempts_per_student, registration_deadline, exam_schedule_id } = data;
    const result = await db.prepare(`
  ...
  `).bind(exam_type_id, level, title, description || null, duration_minutes, passing_score || null, shuffle_questions ? 1 : 0, shuffle_options ? 1 : 0, created_by, requires_registration ? 1 : 0, max_attempts_per_student || null, registration_deadline || null, exam_schedule_id || null).run();
    return result;
}
