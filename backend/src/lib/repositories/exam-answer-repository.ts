...
export async function saveAnswer(db: D1Database, attemptId: number | string, questionId: number | string, answerData: any): Promise<any> {
...
export async function calculateScore(db: D1Database, attemptId: number | string): Promise<{ totalScore: number; maxScore: number; sectionScores: Record<string, any> }> {
  ...
  (answers.results || []).forEach((answer: any) => {
...
export async function getAttemptResult(db: D1Database, attemptId: number | string): Promise<any | null> {
  const attempt = await db.prepare(`
    SELECT * FROM exam_attempts WHERE id = ?
  `).bind(attemptId).first<any>();
  ...
  const answersList = (answers.results || []).map((a: any) => ({
