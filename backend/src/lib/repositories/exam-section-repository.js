...
export async function createSection(db, testId, data) {
    const { name, description, time_limit_minutes, order_index, instructions, is_locked_after_complete, scoring_rule } = data;
    const result = await db.prepare(`
    INSERT INTO exam_sections (
      test_id, name, description, time_limit_minutes, order_index,
      instructions, is_locked_after_complete, scoring_rule
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(testId, name, description, time_limit_minutes, order_index, instructions, is_locked_after_complete ? 1 : 0, scoring_rule || 'points_based').run();
    return result;
}
