...
export async function updateHeartbeat(db, attemptId) {
...
export async function validateAttemptAccess(db, studentId, attemptId) {
  const attempt = await db.prepare(`
...
export async function checkAttemptExpiry(db, attemptId) {
  const attempt = await db.prepare(`
    SELECT * FROM exam_attempts WHERE id = ?
  `).bind(attemptId).first();
...
export async function rateLimitAttempts(db, studentId, testId) {
  ...
  if (test.max_attempts_per_student === null || test.max_attempts_per_student === undefined) {
    ...
    const result = await db.prepare(`
      ...
      AND DATE(created_at) = ?
    `).bind(studentId, testId, today).first();
...
export async function checkTestAccess(db, studentId, testId) {
  const test = await getExamTestById(db, testId);
  if (!test) {
