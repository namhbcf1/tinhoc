 ...
 * @param {object} question - Row from exam_questions
 * @param {*} answerData - User-supplied answer value
 * @returns {{ isCorrect: boolean, pointsEarned: number }}
 */
export function scoreAnswer(question, answerData) {
  ...
  if (question.type === 'multi_select') {
    try {
      const correctAnswers = typeof question.answer_key === 'string' && question.answer_key.startsWith('[')
        ? JSON.parse(question.answer_key).map(a => String(a).trim()).sort()
        : String(question.answer_key).split(',').map(a => a.trim()).sort();
      const userAnswers = Array.isArray(answerData)
        ? answerData.map(a => String(a).trim()).sort()
        : (typeof answerData === 'string' && answerData.startsWith('['))
        ? JSON.parse(answerData).map(a => String(a).trim()).sort()
        : String(answerData || '').split(',').map(a => a.trim()).sort();
  ...
  if (question.type === 'matching') {
    try {
      ...
      const correctSet = new Set(correctPairs.map(p => String(p).toLowerCase()));
      const userSet = new Set(userPairs.map(p => String(p).toLowerCase()));
      const correctCount = Array.from(correctSet).filter(p => userSet.has(p)).length;
