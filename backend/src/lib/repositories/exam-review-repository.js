...
export async function submitTestForReview(db, testId, teacherId) {
...
export async function approveTest(db, testId, adminId, comment = null) {
...
export async function rejectTest(db, testId, adminId, reason) {
    const now = new Date().toISOString();
    const result = await db.prepare(`
    UPDATE exam_tests
...
export async function getPendingTests(db) {
    try {
        const result = await db.prepare(`
...
export async function getTestReviews(db, testId) {
    const result = await db.prepare(`
