...
interface ExamTestFilters {
  exam_type_id?: number | string;
  level?: string;
  is_active?: boolean;
}

interface CreateExamTestData {
  exam_type_id: number | string;
  level: string;
  title: string;
  ...
  max_attempts_per_student?: number | null;
  registration_deadline?: string | null;
  exam_schedule_id?: number | string | null;
}

export async function getExamTests(db: D1Database, filters: ExamTestFilters = {}): Promise<any[]> {
...
export async function getExamTestById(db: D1Database, testId: number | string): Promise<any> {
...
export async function createExamTest(db: D1Database, data: CreateExamTestData): Promise<any> {
  const {
...
export async function updateExamTest(db: D1Database, testId: number | string, data: Record<string, any>): Promise<any> {
...
export async function deleteExamTest(db: D1Database, testId: number | string): Promise<any> {
  const result = await db.prepare(`
