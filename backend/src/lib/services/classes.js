import { getAllClasses, getOpenClasses, getClassById, createClass, updateClass, deleteClass, searchStudents, getAllStudents, findRegistration } from '../../db/queries.js';
import { createClassSchedule, getClassSchedules, deleteClassSchedule } from '../../db/class-schedule-queries.js';
import { deleteCalendarEvent, updateCalendarEventSafe } from '../../services/google-calendar.js';
import { formatDate } from '../../utils/helpers.js';
export async function fetchAllClasses(db) {
    try {
...
export async function fetchOpenClasses(db) {
    try {
...
export async function fetchAvailableStudents(db, classId, keyword) {
    try {
        ...
        const registeredQuery = await db.prepare(`SELECT student_id FROM registrations WHERE class_id = ?`).bind(classId).all();
        const registeredIds = new Set((registeredQuery.results || []).map((r) => r.student_id));
...
export async function addStudentToClass(db, classId, studentId) {
    try {
        const classInfo = await getClassById(db, Number(classId));
        if (!classInfo) {
            return { success: false, error: { message: 'Không tìm thấy lớp', code: 'NOT_FOUND' } };
...
export async function fetchClassById(db, classId) {
    try {
        const classInfo = await getClassById(db, Number(classId));
        if (!classInfo) {
            return { success: false, error: { message: 'Không tìm thấy lớp', code: 'NOT_FOUND' } };
...
export async function addClass(db, data) {
    try {
        if (data.class_type === 'hoc') {
            const required = ['ten_lop', 'ngay_bat_dau', 'ngay_ket_thuc', 'open_at', 'close_at'];
            for (const field of required) {
                if (!data[field])
                    return { success: false, error: { message: `Thiếu trường: ${field}`, code: 'BAD_REQUEST' } };
            }
        ...
        if (data.ngay_bat_dau) {
            try {
                data.ngay_bat_dau = formatDate(data.ngay_bat_dau);
            }
            catch (e) {
                return { success: false, error: { message: `Ngày bắt đầu không hợp lệ: ${data.ngay_bat_dau}`, code: 'BAD_REQUEST' } };
            }
        }
        if (data.ngay_ket_thuc) {
            try {
                data.ngay_ket_thuc = formatDate(data.ngay_ket_thuc);
            }
            catch (e) {
                return { success: false, error: { message: `Ngày kết thúc không hợp lệ: ${data.ngay_ket_thuc}`, code: 'BAD_REQUEST' } };
            }
        }
        if (data.ngay_thi) {
            try {
                data.ngay_thi = formatDate(data.ngay_thi);
            }
            catch (e) {
                return { success: false, error: { message: `Ngày thi không hợp lệ: ${data.ngay_thi}`, code: 'BAD_REQUEST' } };
            }
        }
        if (!data.ngay_thi && data.ngay_bat_dau)
            data.ngay_thi = data.ngay_bat_dau;
        if (data.open_at) {
            const openDate = new Date(data.open_at);
            if (isNaN(openDate.getTime()))
                return { success: false, error: { message: `Thời gian mở đăng ký không hợp lệ: ${data.open_at}`, code: 'BAD_REQUEST' } };
            data.open_at = openDate.toISOString();
        }
        if (data.close_at) {
            const closeDate = new Date(data.close_at);
            if (isNaN(closeDate.getTime()))
                return { success: false, error: { message: `Thời gian đóng đăng ký không hợp lệ: ${data.close_at}`, code: 'BAD_REQUEST' } };
...
export async function modifyClass(env, id, data) {
    try {
        const db = env.DB;
        if (data.ngay_thi)
            data.ngay_thi = formatDate(data.ngay_thi);
        ...
        if (data.ngay_ket_thuc)
            data.ngay_ket_thuc = formatDate(data.ngay_ket_thuc);
        const result = await updateClass(db, Number(id), data);
        if (!result.success) {
            ...
            if (existingSchedules.results) {
                for (const schedule of existingSchedules.results) {
                    if (schedule.google_event_id) {
                        try {
                            await deleteCalendarEvent(env, schedule.google_event_id);
                        }
                        catch (e) { }
                    }
            ...
            try {
                ...
                if (schedules && schedules.results) {
                    for (const schedule of schedules.results) {
                        if (schedule.google_event_id) {
                            try {
                                await updateCalendarEventSafe(env, schedule.google_event_id, { class_name: data.ten_lop });
                            }
                            catch (e) { }
                        }
                    }
...
export async function removeClass(env, id) {
    try {
        const db = env.DB;
        try {
            const schedules = await getClassSchedules(db, Number(id));
        ...
        const result = await deleteClass(db, Number(id));
        if (!result.success) {
