...
export async function fetchAvailableStudents(db, classId, keyword) {
  try {
    const classInfo = await getClassById(db, classId);
    if (!classInfo) {
...
export async function addStudentToClass(db, classId, studentId) {
  try {
    ...
    const existingReg = await findRegistration(db, studentId, classId);
    if (existingReg) {
...
export async function modifyClass(env, id, data) {
  try {
    ...
    const result = await updateClass(db, id, data);
    if (!result.success) {
    ...
    if (data.schedule_days && data.schedule_days.length > 0 && data.schedule_start_time && data.schedule_end_time) {
      const existingSchedules = await getClassSchedules(db, id);
    ...
    if (data.ten_lop) {
      try {
        const schedules = await getClassSchedules(db, id);
...
export async function removeClass(env, id) {
  try {
    ...
    const result = await deleteClass(db, id);
    if (!result.success) {
