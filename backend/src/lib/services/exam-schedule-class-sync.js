import { activateEnrollmentDirect, cancelEnrollment, createClass, deleteClassById, findClassBySourceExamSchedule, findEnrollment, reactivateEnrollment, updateClassById, } from './online-classes.js';
...
function toDateOnlyFromDateTime(value) {
...
function isAutoLearningSessionNote(value) {
    return normalizeString(value) === AUTO_LEARNING_SESSION_NOTE;
}
async function findExamSchedule(db, examScheduleId) {
...
function hasLinkedClassSeed(schedule) {
    return Boolean(schedule.class_seed_name ||
        schedule.class_seed_schedule_rule ||
        schedule.class_seed_schedule_time ||
        schedule.class_seed_start_date);
}
function hasZoomMeetingConfig(schedule) {
    return Boolean(normalizeString(schedule.zoom_link) ||
...
function shouldSyncLinkedClass(schedule) {
    ...
    const canSyncByProgramConfig = Boolean(schedule.delivery_mode === 'internal_training' &&
...
async function resolveExamCategoryId(db, explicitCategoryId, legacyExamType) {
    ...
    const row = await db.prepare(`
      ...
      ORDER BY id
      LIMIT 1
    `).bind(legacy, legacy).first();
    return row?.id ?? null;
}
async function resolveExamTypeId(db, explicitExamTypeId, legacyExamType) {
...
function buildClassSeed(schedule) {
    ...
    return {
        class_name: String(schedule.class_seed_name ||
            `${schedule.exam_name} - Lớp ôn tập`).trim(),
        description: String(schedule.class_seed_description ||
            schedule.notes ||
            `Lớp học được tạo tự động từ lịch thi ${schedule.exam_name}.`).trim(),
...
async function syncLearningSessionsForLinkedClass(db, examScheduleId) {
    ...
    const existingByDate = new Map(existingSessions
        .map((row) => [normalizeString(row.session_date) || '', row])
    ...
    for (const sessionDate of targetDates) {
        ...
        const needsUpdate = normalizeString(existingSession.start_time) !== startTime ||
...
async function cleanupClassScopedData(db, classId) {
    const assignmentIds = await db.prepare(`SELECT id FROM assignments WHERE class_id = ?`).bind(classId).all();
    ...
    await db.prepare(`DELETE FROM document_shares WHERE target_type = 'online_class' AND target_id = ?`).bind(classId).run();
    await db.prepare(`DELETE FROM notifications WHERE online_class_id = ? AND COALESCE(audience_scope, 'all') = 'class' AND source_site = 'edu'`).bind(classId).run();
    await db.prepare(`UPDATE practice_exam_assignments SET online_class_id = NULL WHERE online_class_id = ? AND student_id IS NOT NULL`).bind(classId).run();
    await db.prepare(`DELETE FROM practice_exam_assignments WHERE online_class_id = ? AND student_id IS NULL`).bind(classId).run();
}
export async function syncLinkedOnlineClassForExamSchedule(db, env, examScheduleId, actorId) {
    const schedule = await findExamSchedule(db, examScheduleId);
    ...
    const meetLink = normalizeString(schedule.zoom_link) ||
        normalizeString(schedule.zoom_link_backup) ||
        normalizeString(schedule.zoom_link_backup_2) ||
        normalizeString(schedule.zoom_link_backup_3) ||
        null;
...
export async function syncLinkedClassSessionsForExamSchedule(db, examScheduleId) {
    return syncLearningSessionsForLinkedClass(db, examScheduleId);
}
export async function syncApprovedExamRegistrationsToOnlineClass(db, examScheduleId) {
    const linkedClass = await findClassBySourceExamSchedule(db, examScheduleId);
    if (!linkedClass) {
        return { synced: 0 };
    }
...
export async function syncSingleExamRegistrationToOnlineClass(db, examScheduleId, studentId) {
    const linkedClass = await findClassBySourceExamSchedule(db, examScheduleId);
    if (!linkedClass) {
        return { synced: false };
    }
...
export async function revokeExamRegistrationFromOnlineClass(db, examScheduleId, studentId) {
    const linkedClass = await findClassBySourceExamSchedule(db, examScheduleId);
    if (!linkedClass) {
...
export async function deleteLinkedOnlineClassForExamSchedule(db, env, examScheduleId) {
    const linkedClass = await findClassBySourceExamSchedule(db, examScheduleId);
    if (!linkedClass) {
...
export async function resyncAllLinkedOnlineClasses(db, env, actorId) {
    ...
    for (const linkedClass of linkedClasses.results || []) {
        ...
        WHERE exam_id = ?
          AND status IN ('approved', 'registered')
      `).bind(linkedClass.source_exam_schedule_id).all();
        const approvedStudentIds = new Set((approvedRows.results || [])
