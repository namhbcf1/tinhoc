 ...
 */
import { listClasses, findClassById, insertClass, updateClass, updateClassMeetLink, updateClassCalendarInfo, updateClassCalendarSync, deleteClass, findClassBySourceExamSchedule, countActiveEnrollments, getPendingCountsByClass, findEnrollment, findEnrollmentById, createEnrollment, reEnroll, activateEnrollmentDirect, reactivateEnrollment, approveEnrollment, rejectEnrollment, cancelEnrollment, listEnrolledStudents, listActiveEnrollmentsWithStudents, listPendingEnrollmentsWithStudents, getEnrolledStudentIds, findStudentByCccd, findStudentById, searchStudents } from '../repositories/online-classes.js';
import { createOnlineClassEvent, updateCalendarEventSafe, deleteOnlineClassEvent, getMeetLinkFromEvent, findEventByClassName } from '../../services/google-calendar.js';
// ─── Helpers ────────────────────────────────────────────────────────────────
const SCHEDULE_TIME_RE = /^\d{2}:\d{2}-\d{2}:\d{2}$/;
/**
 * Returns true if current time in `timezone` is within [start - earlyBuf, end + lateBuf].
 */
export function isWithinClassTime(scheduleTime, timezone, earlyBufferMinutes = 0, lateBufferMinutes = 15) {
 ...
 */
async function retryGetMeetLink(env, eventId, maxAttempts = 3) {
 ...
 */
export async function getClassList(db, query, viewer) {
 ...
 */
export async function autoSyncMeetLink(db, env, cls) {
    if (!cls.calendar_event_id || cls.meet_link)
        return null;
    try {
 ...
 */
export async function getClassDetail(db, id, viewer) {
    ...
    const canJoinNow = isWithinClassTime(cls.schedule_time, cls.timezone || 'Asia/Ho_Chi_Minh', 0, 15);
    return {
        ...cls,
 ...
 */
export async function getMyStatus(db, classId, viewer) {
    ...
    const canJoin = isWithinClassTime(cls.schedule_time, cls.timezone || 'Asia/Ho_Chi_Minh', 0, 15);
 ...
 */
export async function createClass(db, env, body, createdBy) {
    const { class_name, description, schedule_rule, schedule_time, timezone = 'Asia/Ho_Chi_Minh', start_date, end_date, teacher_name, max_students = 50, source_exam_schedule_id = null, source_kind = 'exam_schedule', exam_category_id = null, exam_type_id = null, organizer_uuid = null, program_uuid = null, level_uuid = null, custom_field_payload = null, override_payload = null, meet_link: bodyMeetLink = null, } = body;
    // Validation
 ...
 */
export async function updateClassById(db, env, id, body) {
    const existing = await findClassById(db, id);
    if (!existing)
        return null;
    const { class_name, description, schedule_rule, schedule_time, timezone, start_date, end_date, teacher_name, max_students, status, meet_link, source_exam_schedule_id, source_kind, exam_category_id, exam_type_id, organizer_uuid, program_uuid, level_uuid, custom_field_payload, override_payload } = body;
    ...
    if (schedule_rule !== undefined)
        fields.schedule_rule = schedule_rule;
    if (schedule_time !== undefined)
        fields.schedule_time = schedule_time;
    if (timezone !== undefined)
        fields.timezone = timezone || 'Asia/Ho_Chi_Minh';
    if (start_date !== undefined)
        fields.start_date = start_date;
    if (end_date !== undefined)
        fields.end_date = end_date || null;
    if (source_exam_schedule_id !== undefined)
        fields.source_exam_schedule_id = source_exam_schedule_id;
    if (source_kind !== undefined)
        fields.source_kind = source_kind;
    if (exam_category_id !== undefined)
        fields.exam_category_id = exam_category_id;
    if (exam_type_id !== undefined)
        fields.exam_type_id = exam_type_id;
    if (organizer_uuid !== undefined)
        fields.organizer_uuid = organizer_uuid;
    if (program_uuid !== undefined)
        fields.program_uuid = program_uuid;
    if (level_uuid !== undefined)
        fields.level_uuid = level_uuid;
    if (custom_field_payload !== undefined)
        fields.custom_field_payload = custom_field_payload;
    if (override_payload !== undefined)
        fields.override_payload = override_payload;
 ...
 */
export async function deleteClassById(db, env, id) {
    const existing = await findClassById(db, id);
    if (!existing)
        return false;
 ...
 */
export async function regenerateMeetLink(db, env, id) {
    const existing = await findClassById(db, id);
    if (!existing)
        return null;
    // Already complete
 ...
 */
export async function enrollStudent(db, classId, studentId) {
    const cls = await findClassById(db, classId);
    if (!cls)
        throw Object.assign(new Error('Không tìm thấy lớp học'), { statusCode: 404 });
    if (cls.status !== 'active')
        throw Object.assign(new Error('Lớp học không còn mở đăng ký'), { statusCode: 400 });
    ...
    if (existing) {
        if (existing.status === 'active')
            throw Object.assign(new Error('Bạn đã đăng ký lớp học này'), { statusCode: 400 });
        if (existing.status === 'pending')
            throw Object.assign(new Error('Bạn đã đăng ký và đang chờ duyệt'), { statusCode: 400 });
 ...
 */
export async function adminAddStudent(db, classId, studentId) {
    const cls = await findClassById(db, classId);
    if (!cls)
        throw Object.assign(new Error('Không tìm thấy lớp học'), { statusCode: 404 });
    const student = await findStudentById(db, studentId);
    if (!student)
        throw Object.assign(new Error('Không tìm thấy học viên'), { statusCode: 404 });
    const existing = await findEnrollment(db, classId, studentId);
    if (existing) {
        if (existing.status === 'active') {
 ...
 */
export async function approveEnrollmentById(db, classId, enrollmentId, adminId) {
    const enrollment = await findEnrollmentById(db, enrollmentId, classId);
    if (!enrollment)
        throw Object.assign(new Error('Không tìm thấy yêu cầu đăng ký'), { statusCode: 404 });
    if (enrollment.status === 'active')
        throw Object.assign(new Error('Học viên này đã được duyệt rồi'), { statusCode: 400 });
    // Check capacity
    const cls = await findClassById(db, classId);
 ...
 */
export async function rejectEnrollmentById(db, classId, enrollmentId, adminId, reason) {
    const enrollment = await findEnrollmentById(db, enrollmentId, classId);
    if (!enrollment)
        throw Object.assign(new Error('Không tìm thấy yêu cầu đăng ký'), { statusCode: 404 });
    if (enrollment.status === 'rejected')
        throw Object.assign(new Error('Học viên này đã bị từ chối rồi'), { statusCode: 400 });
 ...
 */
export async function removeStudent(db, classId, studentId) {
    const result = await cancelEnrollment(db, classId, studentId);
 ...
 */
export async function findStudentForAuth(db, cccd) {
    return findStudentByCccd(db, cccd);
}
/**
 * Get available students (not yet active-enrolled in class) for admin search.
 */
export async function getAvailableStudents(db, classId, keyword) {
    const cls = await findClassById(db, classId);
    if (!cls)
        return null;
    const students = await searchStudents(db, keyword || '');
    const enrolledIds = await getEnrolledStudentIds(db, classId);
    return (students.results || []).filter((s) => !enrolledIds.has(s.id));
}
