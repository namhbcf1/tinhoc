...
export function buildSubjectKey(principal) {
    return `${principal.userType}:${principal.id}`;
}
function getAppBaseUrl(targetApp, env) {
    if (targetApp === 'exam') {
...
export function buildSessionUser(session) {
    return {
        id: session.user_id,
...
function buildJwtPayload(session, audience, expiresAtSeconds) {
    const payload = {
        id: session.user_id,
...
async function insertSessionRecord(db, principal, sid) {
    ...
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)
    `).bind(sid, buildSubjectKey(principal), principal.id, principal.userType, normalizeString(principal.role), normalizeString(principal.username), normalizeString(principal.cccd), normalizeString(principal.teacherCode), normalizeString(principal.phone), normalizeString(principal.email), normalizeString(principal.displayName), timestamp, timestamp, timestamp).run();
    return getActiveSessionBySid(db, sid);
}
export async function getActiveSessionBySid(db, sid) {
...
export async function touchSession(db, sid) {
    await db.prepare(`
...
export async function issueSessionToken(db, env, principal, audience, existingSid) {
...
export async function ensureSessionForPayload(db, env, payload) {
    ...
    const idValue = typeof payload.id === 'number'
        ? payload.id
    ...
    const issued = await issueSessionToken(db, env, principal, payload.aud || 'edu');
    return issued.session;
}
export async function createSsoHandoff(db, env, session, targetApp, returnTo) {
...
export async function exchangeSsoHandoff(db, env, ticket, targetApp) {
    ...
    const issued = await issueSessionToken(db, env, {
        id: session.user_id,
...
export async function revokeSessionBySid(db, sid) {
    await db.prepare(`
      UPDATE auth_sessions
      SET status = 'revoked',
          revoked_at = ?,
