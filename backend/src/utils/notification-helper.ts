...
// @ts-ignore - JS file not yet migrated to TypeScript
 ...
 */
export async function notifyNewClassOpen(db: D1Database, className: string, classId: string | number): Promise<void> {
  await createNotification(db, {
    user_id: null,
    user_type: 'all',
 ...
 */
export async function notifyAdminPaymentPending(db: D1Database, studentName: string, amount: number, paymentId: string | number): Promise<void> {
  await createNotification(db, {
    user_id: null,
    user_type: 'admin',
