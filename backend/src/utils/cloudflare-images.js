 ...
 */
export async function uploadToCloudflareImages(env, file, metadata = {}, requireSignedURLs = true) {
    try {
 ...
 */
export async function generateSignedImageURL(env, imageId, expiryMinutes = 60) {
    try {
        if (!imageId) {
            throw new Error('Image ID is required');
        }
        const expiryTimestamp = Math.floor(Date.now() / 1000) + (expiryMinutes * 60);
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/images/v1/${imageId}/signed-url`, {
            method: 'POST',
            headers: {
                ...getAuthHeaders(env),
 ...
 */
export async function deleteCloudflareImage(env, imageId) {
    try {
        if (!imageId) {
            return false;
        }
 ...
 */
export async function getImageDetails(env, imageId) {
    try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/images/v1/${imageId}`, {
            headers: {
                ...getAuthHeaders(env)
 ...
 * @returns {Promise<boolean>} Success status
 */
export async function updateImageMetadata(env, imageId, metadata) {
    try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/images/v1/${imageId}`, {
            method: 'PATCH',
            headers: {
 ...
 */
export async function generateMultipleSignedURLs(env, imageIds, expiryMinutes = 60) {
    try {
        const urls = {};
 ...
 */
export function calculateExpiryDatetime(expiryMinutes = 60) {
 ...
 */
export function isURLExpired(expiryDatetime) {
    if (!expiryDatetime)
        return true;
