 ...
 */
export async function sendEmail(options: EmailOptions, env: Env): Promise<{ success: boolean; message?: string }> {
  const { to, subject, html, text } = options;

  try {
    // Option 1: Use Cloudflare Email Workers (if configured)
    if ((env as any).EMAIL_WORKER_URL) {
      const response = await fetch((env as any).EMAIL_WORKER_URL, {
        method: 'POST',
        headers: {
    ...
    if ((env as any).RESEND_API_KEY) {
      const response = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: (env as any).FROM_EMAIL || '[email protected]',
          to: [to],
          subject,
          html,
          text: text || html.replace(/<[^>]*>/g, ''),
