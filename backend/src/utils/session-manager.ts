 ...
 */
export function createSession(userId: string, userData: Record<string, unknown> = {}, expiresIn = 24 * 60 * 60 * 1000): string {
  ...
  return async (c: Context, next: Next) => {
    ...
    if (sessionId) {
      ...
      if (session) {
        ...
        c.set('user' as any, session.userData);
      }
    }
    ...
    const newSessionId = c.get('newSessionId' as any);
    if (newSessionId) {
