...
import { createExamSchedule, getExamSchedulesByClass, getUpcomingExams, updateExamSchedule, deleteExamSchedule, restoreExamSchedule, getDeletedExamSchedules, permanentlyDeleteExamSchedule, cleanupOldDeletedExams, getStudentExams, registerStudentForExam, cancelExamRegistration, getExamRegistrations, getPendingExamRegistrations, updateExamRegistrationPaymentStatus, approveExamRegistration, approveAllExamRegistrations, rejectExamRegistration, isUpcomingExamRegistrationWindow, getZoomCheckinsForExam, } from '../db/attendance-queries.js';
import { checkRegistrationStatus, approveExamTestRegistration, registerForExamTest } from '../db/exam-queries.js';
...
import { deleteLinkedOnlineClassForExamSchedule, resyncAllLinkedOnlineClasses, revokeExamRegistrationFromOnlineClass, syncApprovedExamRegistrationsToOnlineClass, syncLinkedClassSessionsForExamSchedule, syncLinkedOnlineClassForExamSchedule, syncSingleExamRegistrationToOnlineClass, } from '../lib/services/exam-schedule-class-sync.js';
...
import { buildSessionDuplicateKey, isHourMinute, isIsoDate, parseLearningScheduleText, } from '../services/learning-schedule-ocr.js';
...
async function syncLinkedLearningState(db, env, examId, actorId) {
...
function hasExamAdminAccess(user) {
    return Boolean(user && (user.role === 'admin' || user.role === 'super_admin'));
}
function hasExamFeeAdminAccess(user) {
    return hasExamAdminAccess(user)
        && !trimNullable(user?.teacher_code || user?.teacherCode);
}
function sanitizeExamRegistrationFeeStatus(user, students) {
    if (hasExamFeeAdminAccess(user)) {
        return students;
    }
    return students.map(({ payment_status, ...student }) => student);
}
function requireExamAdmin(user, message = 'Chỉ admin mới có quyền truy cập') {
...
function parseExamAnchorYear(examDate) {
    if (!examDate)
        return undefined;
...
async function getLinkedOnlineClassForExam(db, examId) {
    return db.prepare(`
...
async function getExistingSessionDuplicateKeys(db, onlineClassId) {
    const rows = await db.prepare(`
      SELECT session_date, start_time, end_time
      FROM online_class_sessions
      WHERE online_class_id = ?
    `).bind(onlineClassId).all();
    const keys = new Set();
...
function hasUniqueConstraintError(error) {
    const message = String(error?.message || '');
    return /UNIQUE/i.test(message);
}
async function lookupExcelTemplateId(db, name) {
...
async function getOrganizerTemplateContext(db, organizerUuid) {
    if (!organizerUuid) {
    ...
    const result = await db.prepare(`
      ...
      WHERE uuid = ?
      LIMIT 1
    `).bind(organizerUuid).first();
    return {
        organizerCode: result?.code ?? null,
        organizerName: result?.name ?? null,
    };
}
async function getProgramTemplateContext(db, programUuid) {
    if (!programUuid) {
    ...
    const result = await db.prepare(`
      ...
      WHERE uuid = ?
      LIMIT 1
    `).bind(programUuid).first();
    return {
        programCode: result?.code ?? null,
        programName: result?.name ?? null,
    };
}
async function resolveProgramOrganizerIdentifier(db, rawValue) {
...
async function resolveProgramIdentifier(db, rawValue, organizerUuid) {
    const normalized = normalizeLookupKey(rawValue);
    if (!normalized) {
        return null;
    }
    const exactMatch = await db.prepare(`
      ...
      LIMIT 1
    `).bind(organizerUuid, organizerUuid, normalized, normalized, normalized).first();
    ...
    const fuzzyMatches = await db.prepare(`
      ...
      LIMIT 2
    `).bind(organizerUuid, organizerUuid, fuzzyToken, fuzzyToken).all();
...
async function inferSingleProgramIdentifierForOrganizer(db, organizerUuid) {
    if (!organizerUuid) {
        return null;
    }
    const results = await db.prepare(`
      ...
      ORDER BY visible_on_edu_admin DESC, id ASC
      LIMIT 2
    `).bind(organizerUuid).all();
    const uuids = (results.results || [])
        .map((row) => trimNullable(row?.uuid))
        .filter(Boolean);
    return uuids.length === 1 ? uuids[0] : null;
}
async function resolveProgramLevelIdentifier(db, rawValue, programUuid) {
    const normalized = normalizeLookupKey(rawValue);
...
function normalizeZoomLinkPair(rawInput, shouldPersistZoom) {
...
function parseTimeRangeBoundary(value, fieldName) {
...
function normalizeClassSeedScheduleTime(value) {
    if (!value) {
        return null;
    }
...
function normalizeClassSeedScheduleRule(value) {
    if (!value) {
        return null;
    }
...
function buildDefaultClassSeedScheduleTime(examDate, durationMinutes) {
...
async function resolveLegacyExamTypeLabel(db, examCategoryId, examTypeId, fallbackExamType) {
    if (examTypeId) {
        const typeRow = await db.prepare(`
        SELECT name, code
        FROM exam_types
        WHERE id = ?
      `).bind(examTypeId).first();
        if (typeRow?.name) {
            return typeRow.name;
        }
        if (typeRow?.code) {
...
async function normalizeExamSchedulePayload(db, rawInput) {
    ...
    const organizerUuid = (await resolveProgramOrganizerIdentifier(db, organizerInput)) ??
        organizerInput;
    const programInput = trimNullable(rawInput?.program_uuid);
    const programUuid = (await resolveProgramIdentifier(db, programInput, organizerUuid)) ??
    ...
    const levelUuid = (await resolveProgramLevelIdentifier(db, levelInput, programUuid)) ??
        levelInput;
    ...
    const examLevel = examLevelCandidate && EXAM_LEVEL_OPTIONS.has(examLevelCandidate)
        ? examLevelCandidate
        : null;
    const classSeedMaxStudents = parseOptionalInt(classSeed.max_students ?? rawInput?.class_seed_max_students, 'class_seed_max_students') ?? 50;
    ...
    const enableLinkedClassFlag = parseOptionalBoolean(rawInput?.enable_linked_class) ??
        parseOptionalBoolean(classSeed.enabled) ??
        null;
    const enableZoomMeetingFlag = parseOptionalBoolean(rawInput?.enable_zoom_meeting) ?? null;
    const hasZoomFields = Boolean(trimNullable(rawInput?.zoom_link) ||
    ...
    const hasLinkedClassSeed = Boolean(classSeedName ||
        classSeedScheduleRule ||
        classSeedScheduleTime ||
        classSeedStartDate);
    const shouldPersistLinkedClass = !isExternalRedirectProgram && (enableLinkedClassFlag === true ||
    ...
    const normalizedClassSeedScheduleRule = shouldPersistLinkedClass
        ? normalizeClassSeedScheduleRule(resolvedClassSeedScheduleRuleInput)
        : null;
    const normalizedClassSeedScheduleTime = shouldPersistLinkedClass
        ? normalizeClassSeedScheduleTime(resolvedClassSeedScheduleTimeInput)
        : null;
    const normalizedClassSeedStartDate = shouldPersistLinkedClass && resolvedClassSeedStartDateInput
        ? toDateOnly(resolvedClassSeedStartDateInput)
        : null;
    const normalizedClassSeedEndDate = shouldPersistLinkedClass && classSeedEndDate
    ...
    if (normalizedClassSeedStartDate &&
    ...
    const examTypeLegacy = await resolveLegacyExamTypeLabel(db, examCategoryId, examTypeId, trimNullable(rawInput?.exam_type));
...
examSchedules.post('/resync-classes', async (c) => {
    try {
        ...
        createActivityLog(c.env.DB, user.id, 'resync_exam_schedule_classes', 'online_classes', null, 'Resynced linked classes from exam schedules', c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'), c.req.header('User-Agent')).catch((err) => console.error('Activity log error:', err));
        return jsonResponse({
            success: true,
...
examSchedules.get('/conflicts', async (c) => {
    ...
    `).all();
        ...
        for (const studentRegistrations of registrationsByStudent.values()) {
            const conflictingRegistrations = studentRegistrations.filter((registration) => isUpcomingExamRegistrationWindow(registration));
            if (conflictingRegistrations.length <= 1) {
                continue;
            }
            const first = conflictingRegistrations[0];
            grouped.set(String(first.student_id), {
                student_id: first.student_id,
                ho_ten_full: first.ho_ten_full,
                cccd: first.cccd,
                active_registrations: conflictingRegistrations.map((registration) => ({
...
examSchedules.post('/', async (c) => {
    try {
        ...
        const result = await createExamSchedule(c.env.DB, payload.class_id, payload.exam_name, payload.exam_date, payload.duration_minutes, payload.location, payload.notes, payload.template_id, payload);
        ...
        createActivityLog(c.env.DB, user.id, 'create_exam_schedule', 'exam_schedules', scheduleId, `Created exam schedule: ${payload.exam_name}`, c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'), c.req.header('User-Agent')).catch(err => console.error('Activity log error:', err));
        return jsonResponse({
            success: true,
...
examSchedules.put('/:id', async (c) => {
    try {
        ...
        createActivityLog(c.env.DB, user.id, 'update_exam_schedule', 'exam_schedules', examId, `Updated exam schedule`, c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'), c.req.header('User-Agent')).catch(err => console.error('Activity log error:', err));
        return jsonResponse({
            success: true,
...
examSchedules.delete('/:id', async (c) => {
    try {
        ...
        // Log activity (fire-and-forget)
        createActivityLog(c.env.DB, user.id, 'delete_exam_schedule', 'exam_schedules', examId, `Moved exam schedule to trash`, c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'), c.req.header('User-Agent')).catch(err => console.error('Activity log error:', err));
        return jsonResponse({
            success: true,
...
examSchedules.post('/:id/restore', async (c) => {
    try {
        ...
        await syncLinkedLearningState(c.env.DB, c.env, examId, user.id);
        // Log activity (fire-and-forget)
        createActivityLog(c.env.DB, user.id, 'restore_exam_schedule', 'exam_schedules', examId, `Restored exam schedule from trash`, c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'), c.req.header('User-Agent')).catch(err => console.error('Activity log error:', err));
        return jsonResponse({
            success: true,
...
examSchedules.delete('/:id/permanent', async (c) => {
    try {
        ...
        // Log activity (fire-and-forget)
        createActivityLog(c.env.DB, user.id, 'permanent_delete_exam_schedule', 'exam_schedules', examId, `Permanently deleted exam schedule`, c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'), c.req.header('User-Agent')).catch(err => console.error('Activity log error:', err));
        return jsonResponse({
            success: true,
...
examSchedules.get('/:id/students', async (c) => {
    try {
        ...
        const enrichedStudents = await Promise.all(students.map((student) => enrichStudentWithImages(c, student)));
        // Nếu yêu cầu thêm thông tin zoom check-in (?with_zoom_checkin=1)
        if (withZoomCheckin) {
            const zoomMap = await getZoomCheckinsForExam(c.env.DB, examId);
            const studentsWithZoom = enrichedStudents.map((student) => {
...
examSchedules.get('/:id/learning-attendance', async (c) => {
    ...
    `).bind(...sessionIds).all();
        const attRows = (attResult.results || []);
        ...
        const studentRows = students.map((s) => {
            ...
            const sessionAttendance = sessions.map((sess) => {
                ...
                const isCounted = Boolean(sessionDateKey &&
                    registrationDate &&
...
examSchedules.post('/:id/learning-sessions/import-commit', async (c) => {
    try {
        ...
        for (const row of normalizedRows) {
            ...
            VALUES (?, ?, ?, ?, ?, ?)
          `).bind(onlineClassId, row.session_date, row.start_time, row.end_time, row.note, user.id ?? null).run();
...
examSchedules.put('/:id/students/:studentId/payment-status', async (c) => {
    try {
        ...
        const result = await updateExamRegistrationPaymentStatus(c.env.DB, parseInt(id), parseInt(studentId), paymentStatus);
...
examSchedules.post('/:id/learning-sessions', async (c) => {
    ...
    `).bind(examId).first();
      ...
      INSERT INTO online_class_sessions (online_class_id, session_date, start_time, end_time, note, created_by)
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(onlineClass.id, body.session_date, body.start_time, body.end_time, body.note ?? null, user.id ?? null).run();
        return jsonResponse({
            success: true,
...
examSchedules.put('/:id/learning-sessions/:sessionId/attendance/:studentId', async (c) => {
    ...
    `).bind(sessId, stdId, body.status, body.note ?? null, body.status === 'present' || body.status === 'late' ? now : null, user.id ?? null, user.role ?? 'admin', now).run();
