...
const grading = new Hono<{ Bindings: Env; Variables: { user: JWTPayload } }>();
...
const gradingAccess = async (c: any, next: any) => {
...
grading.post('/submit', authMiddleware, gradingAccess, async (c) => {
  ...
  try {
    ...
    const scores: Record<string, number | null> = { LISTENING: null, READING: null, WRITING: null, SPEAKING: null };
    for (const row of sectionTotals.results as any[]) {
      scores[row.section_type] = row.section_score;
    }
    const total = Object.values(scores).filter(v => v !== null).reduce((a, b) => (a as number) + (b as number), 0) as number;
