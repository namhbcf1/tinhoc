...
auth.post('/create-admin', async (c) => {
  try {
    const { username, password, full_name, role } = await c.req.json() as any;
...
auth.post('/forgot-password', async (c) => {
  try {
    ...
    try {
      ...
      const adminEmail = (c.env as any).ADMIN_EMAIL || admin.username + '@sontrang.edu.vn';
...
auth.post('/change-password', async (c) => {
  try {
    ...
    // Get admin
    const admin = await findAdminById(c.env.DB, (payload as any).id) as any;
