...
import { uploadToCloudflareImages, validateImageFile, generateSignedImageURL, deleteCloudflareImage, } from '../utils/cloudflare-images.js';
import { extractRegistrationPrefillFromCandidates, extractRegistrationPrefillFromImage, } from '../services/cccd-ocr-service.js';
import { enqueuePhoto3x4Pipeline, getPhoto3x4ProcessingStatus, regeneratePhoto3x4Pipeline, runPhoto3x4Pipeline, selectPhoto3x4Variant, } from '../services/photo-3x4-pipeline.js';
...
function getMissingCriticalFields(prefill, type) {
    if (type === 'cccd_front') {
        return [
    ...
    return [!prefill.issueDate ? 'ngày cấp' : null].filter(Boolean);
}
const VALID_TYPES = ['cccd_front', 'cccd_back', 'photo_3x4'];
const PIPELINE_VERSION = 'v2-generate-first';
const app = new Hono();
function hasCloudflareImages(env) {
    return Boolean(env.CLOUDFLARE_IMAGES_API_TOKEN && env.CLOUDFLARE_ACCOUNT_ID);
}
function buildR2PreviewUrl(imageKey) {
    return `/api/students/image/${encodeURIComponent(imageKey)}`;
}
function buildPipelineSourceUrl(c, imageKey) {
    try {
...
function readOptionalFormFile(value) {
    if (value instanceof File && value.size > 0) {
        return value;
    }
    return null;
}
async function storeAuxiliaryDocumentVariant(c, file, type, processingLogId, variantKey, studentId) {
...
async function buildPreviewUrl(c, imageId, fallbackUrl) {
    if (!imageId)
...
app.post('/', async (c) => {
    try {
        ...
        if (shouldUseCloudflareImages) {
            const uploaded = await uploadToCloudflareImages(c.env, file, metadata, true);
            uploadResult = {
                success: true,
      ...
      `).bind(studentId || null, type, uploadResult.imageId, uploadResult.imageId, type === 'photo_3x4' ? 'processing' : 'pending', type === 'photo_3x4' ? 'processing' : null, 'uploaded', type === 'photo_3x4' ? 5 : 0, type === 'photo_3x4'
        ...
        if (processingLogId
            ...
            for (const [variantKey, variantFile] of auxiliaryFiles) {
                ...
                try {
                    auxiliaryImageIds[variantKey] = await storeAuxiliaryDocumentVariant(c, variantFile, type, processingLogId, variantKey, studentId);
                }
        ...
        if (processingLogId && (type === 'cccd_front' || type === 'cccd_back')) {
          ...
          WHERE id = ?
        `).bind(finalStatus, auxiliaryImageIds.sourceOriginal || uploadResult.imageId, recommendedImageId || auxiliaryImageIds.normalizedOriginal || uploadResult.imageId, candidateImageId || auxiliaryImageIds.normalizedOriginal || uploadResult.imageId, uploadResult.imageId, detectionConfidence, qualityScore, errorMessage, generationMode, warnings.length > 0 ? JSON.stringify(warnings) : null, validationPayload, JSON.stringify(detailPayload), processingLogId).run();
            }
            catch (aiError) {
        ...
        SET processing_status = 'success',
            ...
            processing_completed_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `).bind(uploadResult.imageId, uploadResult.imageId, uploadResult.imageId, processingLogId).run();
        }
        return c.json({
            success: true,
...
app.post('/extract', async (c) => {
    try {
      ...
      ORDER BY id DESC
      LIMIT 1
    `).bind(imageId, imageId, imageId, imageId).first();
        const processingDetails = safeJsonParse(logRow?.processing_details || null);
        const validationDetails = safeJsonParse(logRow?.validation_result_json || null);
        const restorationArtifacts = (processingDetails?.restorationArtifacts
            || validationDetails?.restorationArtifacts
            || {});
        const recommendedCandidate = String(processingDetails?.recommendedCandidate
        ...
        const hasUsefulData = Boolean(prefill.cccd
            || prefill.fullName
            || prefill.dateOfBirth
            || prefill.gender
            || prefill.ethnicity
