...
payments.get('/stats', async (c) => {
  try {
    ...
    const stats = await getPaymentStats(c.env.DB, { fromDate, toDate });
    ...
    try {
      ...
      if (paymentWithInfo) {
        await notifyPaymentConfirmed(
          c.env.DB,
          paymentWithInfo.student_id,
          paymentWithInfo.amount,
          paymentWithInfo.ten_lop
        );
      }
    } catch (notifError) {
      console.error('Error creating notification:', notifError);
...
payments.get('/registration/:id', async (c) => {
  try {
    ...
    const payments = await getPaymentsByRegistration(c.env.DB, registrationId);
