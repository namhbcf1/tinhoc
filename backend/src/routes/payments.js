...
import { createPayment, getPaymentsByRegistration, getPaymentById, updatePaymentStatus, getPaymentStats, } from '../db/payment-queries.js';
import { updateRegistrationStatus } from '../db/queries.js';
...
payments.get('/stats', async (c) => {
    ...
    `).first();
        ...
        return jsonResponse({
            success: true,
            data: {
                totalRevenue: stats?.total_confirmed || 0, // Tổng doanh thu = tổng đã xác nhận
                paidAmount: stats?.total_confirmed || 0, // Đã thu = tổng đã xác nhận
                unpaidAmount: stats?.total_pending || 0, // Chưa thu = tổng đang chờ
                pendingCount: pendingResult?.count || 0, // Số lượng chờ xử lý
                overdueCount: overdueResult?.count || 0, // Số lượng quá hạn
...
payments.post('/', async (c) => {
    try {
      ...
      `).bind(registration_id).first();
            if (registration) {
                await notifyAdminPaymentPending(c.env.DB, registration.ho_ten_full, amount, result.meta.last_row_id);
            }
        }
        catch (notifError) {
...
payments.put('/:id/confirm', async (c) => {
    try {
        ...
        await updatePaymentStatus(c.env.DB, id, 'confirmed', user?.id);
        // *** AUTO UPDATE REGISTRATION STATUS TO 'STUDYING' ***
        // Khi học viên đã nộp học phí -> tự động cho vào học
        try {
            await updateRegistrationStatus(c.env.DB, payment.registration_id, 'studying');
            console.log(`Auto-updated registration ${payment.registration_id} to 'studying'`);
        }
        catch (statusError) {
      ...
      `).bind(id).first();
            if (paymentWithInfo) {
                await notifyPaymentConfirmed(c.env.DB, paymentWithInfo.student_id, paymentWithInfo.amount, paymentWithInfo.ten_lop);
            }
        }
        catch (notifError) {
            console.error('Error creating notification:', notifError);
...
payments.put('/:id/reject', async (c) => {
    try {
        ...
        await updatePaymentStatus(c.env.DB, id, 'rejected', user?.id);
