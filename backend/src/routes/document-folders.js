...
folders.post('/', authMiddleware, async (c) => {
    try {
        ...
        if (!name)
            return errorResponse('Thiếu name', 400);
        // RBAC
        ...
        if (folder_type === 'private') {
            ...
            if (!isAdmin(user)) {
                if (owner_role && owner_role !== 'admin')
                    return errorResponse('owner_role không hợp lệ', 400);
                if (owner_id && String(owner_id) !== String(user.id))
      ...
      INSERT INTO document_folders (name, parent_id, folder_type, owner_role, owner_id, created_by_admin_id)
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(name, parent_id ? parseInt(parent_id) : null, folder_type, finalOwnerRole, finalOwnerId, createdByAdminId).run();
...
folders.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        if (!existing)
            return errorResponse('Folder không tồn tại', 404);
        // RBAC: shared only admin; private only owner or admin
        if (existing.folder_type === 'shared' && !isAdmin(user))
            return errorResponse('Không có quyền', 403);
        if (existing.folder_type === 'private' && !isAdmin(user)) {
            if (String(existing.owner_id) !== String(user.id) || !['admin', 'teacher'].includes(existing.owner_role))
                return errorResponse('Không có quyền', 403);
        }
        await db.prepare(`
...
folders.delete('/:id', authMiddleware, async (c) => {
    try {
        ...
        if (child)
            return errorResponse('Folder còn folder con, không thể xóa', 400);
        ...
        if (doc)
            return errorResponse('Folder còn tài liệu, không thể xóa', 400);
