...
import {
  getViettelPostErrorMessage,
  listViettelPostDistricts,
  listViettelPostProvinces,
  listViettelPostWards,
  quoteViettelPostShipment,
} from '../services/viettel-post.js';

const shipping = new Hono<{ Bindings: Env; Variables: { user: JWTPayload } }>();
...
shipping.post('/viettel-post/normalize-address', async (c) => {
  try {
    ...
    const provinces = await listViettelPostProvinces(c.env);
    const normalized = await resolveAddressAgainstCatalog(
      rawAddress,
      provinces,
      (provinceId) => listViettelPostDistricts(c.env, provinceId),
      (districtId) => listViettelPostWards(c.env, districtId),
    );
