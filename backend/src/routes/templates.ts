...
const templatesRoute = new Hono<{ Bindings: Env; Variables: { user: JWTPayload; teacher: JWTPayload } }>();
...
templatesRoute.get('/', async (c) => {
    try {
        const templates = await c.env.DB.prepare(
            'SELECT id, name, display_name, is_active FROM excel_templates WHERE is_active = 1 ORDER BY display_name'
        ).all();
