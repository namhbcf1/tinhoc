...
const admins = new Hono<{ Bindings: Env; Variables: { user: JWTPayload } }>();
...
admins.get('/', async (c) => {
  try {
    ...
    await createActivityLog(
      ...
      'admins',
      null,
      `Viewed ${adminsList.length} admins`,
...
admins.get('/:id', async (c) => {
  try {
    ...
    if ((user as any).id !== parseInt(id) && (user as any).role !== 'super_admin') {
      return errorResponse('Không có quyền truy cập', 403);
    }
...
admins.post('/', async (c) => {
  try {
    ...
    const { username, password, full_name, role, email, phone } = await c.req.json() as any;
...
admins.put('/:id', async (c) => {
  try {
    const user = c.get('user');
    const { id } = c.req.param();
    const updateData = await c.req.json() as any;
    ...
    await createActivityLog(
      ...
      'admins',
      adminId,
      `Updated admin: ${JSON.stringify(updateData)}`,
      c.req.header('CF-Connecting-IP') || c.req.header('X-Forwarded-For'),
      c.req.header('User-Agent')
    );
