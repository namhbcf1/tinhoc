...
const studentReviews = new Hono<{ Bindings: Env; Variables: { user: JWTPayload } }>();

// ─────────────────────────────────────────────────────────
// Helpers — inline queries (không tách file riêng vì nhỏ)
// ─────────────────────────────────────────────────────────

async function getReviewFull(db: D1Database, reviewId: number) {
...
async function attachChildren(db: D1Database, review: Record<string, unknown>) {
  ...
  return {
    ...review,
    homework_tracking: JSON.parse((review.homework_tracking as string) || '[]'),
    skills: skills.results,
    test_scores: scores.results,
  };
}

async function syncSkills(db: D1Database, reviewId: number, skills: any[]) {
  await db.prepare('DELETE FROM student_review_skills WHERE review_id = ?').bind(reviewId).run();
  for (let i = 0; i < skills.length; i++) {
    const s = skills[i];
    await db.prepare(
      `INSERT INTO student_review_skills
         (review_id, skill, score_raw, score_num, skill_status, comments, sort_order)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      reviewId, s.skill,
      s.score_raw ?? null, s.score_num ?? null,
      s.skill_status ?? null, s.comments ?? null,
      s.sort_order ?? i,
    ).run();
  }
}

async function syncTestScores(db: D1Database, reviewId: number, scores: any[]) {
  await db.prepare('DELETE FROM student_review_test_scores WHERE review_id = ?').bind(reviewId).run();
  for (let i = 0; i < scores.length; i++) {
    const r = scores[i];
    await db.prepare(
      `INSERT INTO student_review_test_scores
         (review_id, skill_label, max_score, student_score, score_notes, sort_order)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(
      reviewId, r.skill_label,
      r.max_score ?? null, r.student_score ?? null,
      r.score_notes ?? null, r.sort_order ?? i,
    ).run();
  }
}
...
studentReviews.get('/my', requireAuth, async (c) => {
  try {
    ...
    const reviews = await Promise.all(
      rows.results.map((r: any) => attachChildren(c.env.DB, r))
    );
    return successResponse(reviews) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi khi tải báo cáo', 500) as Response;
  }
});
...
studentReviews.get('/class/:classId', requireAdmin, async (c) => {
  try {
    ...
    if (isNaN(classId)) return errorResponse('classId không hợp lệ', 400) as Response;
    ...
    return successResponse(rows.results) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi server', 500) as Response;
  }
});
...
studentReviews.get('/class/:classId/student/:studentId', requireAdmin, async (c) => {
  try {
    ...
    if (isNaN(classId) || isNaN(studentId)) return errorResponse('Tham số không hợp lệ', 400) as Response;
    ...
    if (!row) return successResponse(null) as Response;
    return successResponse(await attachChildren(c.env.DB, row)) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi server', 500) as Response;
  }
});
...
studentReviews.post('/class/:classId/student/:studentId', requireAdmin, async (c) => {
  try {
    ...
    const {
      period_label, report_title, overall_summary, recommendations,
      homework_tracking = [], skills = [], test_scores = [],
    } = body;
    ...
    `).bind(
      studentId, classId,
      period_label ?? null, report_title ?? null,
      overall_summary ?? null, recommendations ?? null,
      homeworkJson, user.id, user.id,
    ).run();
    ...
    if (!review) return errorResponse('Không tạo được báo cáo', 500) as Response;

    await syncSkills(c.env.DB, review.id, skills);
    await syncTestScores(c.env.DB, review.id, test_scores);

    return successResponse(await getReviewFull(c.env.DB, review.id), 201) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi server', 500) as Response;
  }
});
...
studentReviews.put('/:id/publish', requireAdmin, async (c) => {
  try {
    ...
    ).bind('published', user.id, id).run();
    return successResponse({ id, status: 'published' }) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi server', 500) as Response;
  }
});
...
studentReviews.put('/:id/unpublish', requireAdmin, async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      'UPDATE student_reviews SET status = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
    ).bind('draft', user.id, id).run();
    return successResponse({ id, status: 'draft' }) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi server', 500) as Response;
  }
});
...
studentReviews.put('/:id', requireAdmin, async (c) => {
  try {
    ...
    `).bind(
      period_label ?? null, report_title ?? null,
      overall_summary ?? null, recommendations ?? null,
      homework_tracking ? JSON.stringify(homework_tracking) : null,
      user.id, id,
    ).run();

    if (skills) await syncSkills(c.env.DB, id, skills);
    if (test_scores) await syncTestScores(c.env.DB, id, test_scores);

    return successResponse(await getReviewFull(c.env.DB, id)) as Response;
  } catch (err: any) {
    return errorResponse(err?.message || 'Lỗi server', 500) as Response;
  }
});
