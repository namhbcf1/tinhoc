...
import { createCertificate, getCertificateById, updateCertificateStatus, } from '../db/certificate-queries.js';
import { createCertificateShipment, getLatestShipmentByCertificate, getOpenShipmentByCertificate, updateCertificateShipment, } from '../db/certificate-shipment-queries.js';
...
function isIssuedCertificateStatus(status) {
...
async function getClassInfo(db, classId) {
    return db.prepare('SELECT * FROM classes WHERE id = ?').bind(classId).first();
}
async function buildCertificateNumber(db, classId) {
    const certCount = await db.prepare(`
    SELECT COUNT(*) AS count
    FROM certificates
    WHERE class_id = ?
  `).bind(classId).first();
    const count = Number(certCount?.count || 0);
    return `VT-${new Date().getFullYear()}-${classId}-${String(count + 1).padStart(4, '0')}`;
}
async function issueCertificateForStudent(env, classId, studentId, issuedBy, options = {}) {
  ...
  `).bind(studentId, classId).first();
    ...
    try {
        await notifyCertificateIssued(env.DB, String(studentId), certificateNumber, classInfo.ten_lop);
    }
    catch (notifError) {
...
certificates.get('/', async (c) => {
    try {
        ...
        const offset = Number.parseInt(c.req.query('offset'), 10) || 0;
    ...
    try {
        ...
        for (const studentId of student_ids) {
            ...
            if (!normalizedStudentId)
                continue;
            const created = await issueCertificateForStudent(c.env, classId, normalizedStudentId, Number(user?.id) || null, { skipExisting: true });
            if (created) {
...
certificates.post('/:id/shipment', async (c) => {
    const certificateId = Number.parseInt(c.req.param('id'), 10);
    let draftShipmentId = null;
    try {
        const cert = await getCertificateById(c.env.DB, certificateId);
        if (!cert) {
            return errorResponse('Không tìm thấy chứng chỉ', 404);
        }
        if (!isIssuedCertificateStatus(cert.status)) {
        ...
        const serviceAddCodes = Array.isArray(body?.service_add_codes)
            ? body.service_add_codes.map((value) => String(value || '').trim()).filter(Boolean)
            : [];
        ...
        const draftPayload = {
            ...
            student_id: Number(cert.student_id),
