...
vstep.get('/exams', async (c) => {
    try {
            ...
            if (authHeader) {
                ...
                if (user && user.type === 'student') {
                    ...
                    if (regs && regs.length > 0) {
                        classTypeFilter = regs.map(r => r.class_type.toLowerCase());
                    }
                }
            }
        } catch {
        ...
        const exams = await getVStepExams(c.env.DB, {
            status,
            exam_type: classTypeFilter, // null = no filter (admin/no token/no registrations)
        });
...
vstep.get('/exams/:id', authMiddleware, async (c) => {
    try {
        ...
        const data = await getVStepFullExamData(c.env.DB, id);
            ...
            data.sections.forEach(sec => {
                if (sec.groups) {
                    sec.groups.forEach(g => {
                        g.questions.forEach(q => delete q.correct_answer);
                    });
                }
                if (sec.standalone_questions) {
                    sec.standalone_questions.forEach(q => delete q.correct_answer);
                }
            });
        }
...
vstep.get('/attempts/:id', authMiddleware, async (c) => {
    try {
        const { id } = c.req.param();
        const attempt = await getVStepAttempt(c.env.DB, id);
...
vstep.post('/attempts/:id/save', authMiddleware, async (c) => {
    try {
        ...
        await saveVStepAnswer(c.env.DB, id, question_id, answer_text);
...
vstep.post('/attempts/:id/submit', authMiddleware, async (c) => {
    try {
        const { id } = c.req.param();
        await submitVStepAttempt(c.env.DB, id);
