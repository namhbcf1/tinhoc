...
import { findAdminByTeacherCode, findAdminById, createTeacherAdmin, updateAdmin, getAllStaffTeachers, searchStaffTeachers, } from '../db/admin-queries.js';
...
teachers.put('/:id', adminAuthMiddleware, async (c) => {
    try {
        ...
        if (updateData.ho || updateData.ten_dem || updateData.ten) {
            const currentTeacher = await findAdminById(c.env.DB, id);
            if (currentTeacher) {
