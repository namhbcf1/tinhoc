...
import { createClassSchedule, getClassSchedules, updateClassSchedule, deleteClassSchedule } from '../db/class-schedule-queries.js';
import { createCalendarEvent, updateCalendarEvent, deleteCalendarEvent } from '../services/google-calendar.js';
import { requireAdmin } from '../middleware/auth-middleware.js';
const classSchedules = new Hono();
// ========================================
...
classSchedules.post('/', adminAuthMiddleware, async (c) => {
    try {
        ...
        const { class_id, day_of_week, start_time, end_time, room, notes, create_meet_link, class_name, ma_lop, is_recurring, start_date } = data;
...
classSchedules.put('/:id', adminAuthMiddleware, async (c) => {
    try {
        ...
        const schedule = await c.env.DB.prepare('SELECT * FROM class_schedules WHERE id = ?').bind(id).first();
        if (!schedule) {
