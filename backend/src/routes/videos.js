 ...
 * @param {string} options.objectKey - Object key (path in bucket)
 * @param {string} options.accessKeyId - R2 API access key ID
 * @param {string} options.secretAccessKey - R2 API secret access key
 * @param {number} options.expiresIn - URL expiration in seconds (max 7 days)
...
async function ensureCanAccessClass(c, classId, user) {
  ...
  if (user.role === 'teacher' && user.id) {
    const teacherClasses = await getClassTeachers(c.env.DB, classId);
    const assigned = (teacherClasses.results || []).some(
      (row) => String(row.teacher_id) === String(user.id)
    );
    if (!assigned) {
