 ...
 */
async function createR2PresignedUrl({ accountId, bucketName, objectKey, accessKeyId, secretAccessKey, expiresIn = 900, // 15 minutes default
 }) {
...
async function ensureCanAccessClass(c, classId, user) {
    ...
    if (user.type === 'student' && user.id) {
        ...
        const hasClass = registrations.some((r) => String(r.class_id) === String(classId) ||
    ...
    if (user.role === 'teacher' && user.id) {
        ...
        const assigned = (teacherClasses.results || []).some((row) => String(row.teacher_id) === String(user.id));
        if (!assigned) {
...
videos.get('/classes/:class_id/videos', async (c) => {
    try {
        ...
        const stmt = c.env.DB.prepare(`SELECT id, title, duration, created_at
       FROM class_videos
       WHERE class_id = ?
       ORDER BY created_at DESC`).bind(classId);
...
videos.post('/videos/:video_id/play', async (c) => {
    try {
        ...
        if (error)
            return error;
        // Lấy metadata video
        const video = await c.env.DB.prepare(`SELECT id, class_id, r2_key
       FROM class_videos
       WHERE id = ?`).bind(videoId).first();
        if (!video) {
            return errorResponse('Không tìm thấy video', 404);
        }
        // Kiểm tra quyền truy cập lớp
        const membership = await ensureCanAccessClass(c, video.class_id, user);
        if (membership.error)
            return membership.error;
        // ========================================
        ...
        try {
            const playUrl = await createR2PresignedUrl({
                accountId,
                bucketName,
                objectKey: video.r2_key,
                accessKeyId,
                secretAccessKey,
                expiresIn: TTL_SECONDS,
            });
            console.log(`Generated presigned URL for video ${videoId}, user ${user.id || user.cccd}, expires in ${TTL_SECONDS}s`);
            return jsonResponse({
                success: true,
                data: {
                    play_url: playUrl,
...
videos.post('/admin/videos', async (c) => {
    try {
        ...
        await c.env.DB.prepare(`INSERT INTO class_videos (id, class_id, title, r2_key, duration, created_at)
       VALUES (?, ?, ?, ?, ?, datetime('now'))`).bind(id, class_id, title || null, r2_key, duration || null).run();
        return jsonResponse({
            success: true,
...
videos.delete('/admin/videos/:video_id', async (c) => {
    try {
        ...
        const video = await c.env.DB.prepare(`SELECT id, r2_key FROM class_videos WHERE id = ?`).bind(videoId).first();
        if (!video) {
            return errorResponse('Không tìm thấy video', 404);
        }
        // Delete from D1
        await c.env.DB.prepare(`DELETE FROM class_videos WHERE id = ?`).bind(videoId).run();
