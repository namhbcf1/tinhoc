// backend/src/routes/exam-management.ts
...
const exam = new Hono<{ Bindings: Env; Variables: { user: JWTPayload } }>();
...
exam.get('/:id/stats', authMiddleware, adminOnly, async (c) => {
  ...
  try {
    ...
    const qCount = await db.prepare('SELECT COUNT(*) as cnt FROM exam_questions WHERE exam_id = ?').bind(id).first() as any;
    const sCount = await db.prepare('SELECT COUNT(*) as cnt FROM exam_sections WHERE exam_id = ?').bind(id).first() as any;
...
exam.post('/:id/duplicate', authMiddleware, adminOnly, async (c) => {
  ...
  const { id } = c.req.param();
  try {
    const src = await db.prepare('SELECT * FROM exams WHERE id = ?').bind(id).first() as any;
...
exam.put('/:id/sections', authMiddleware, adminOnly, async (c) => {
  ...
  const body = await c.req.json();
  const allowed: Record<string, string> = { type: 'type', title: 'title', duration: 'duration', instructions: 'instructions', orderIndex: 'order_index' };
  const updates: string[] = [];
  const params: any[] = [];
...
exam.put('/:id/groups', authMiddleware, adminOnly, async (c) => {
  ...
  const body = await c.req.json();
  const map: Record<string, string> = { sectionId: 'section_id', title: 'title', textContent: 'text_content', audioUrl: 'audio_url', imageUrl: 'image_url', orderIndex: 'order_index', settingsJson: 'settings_json' };
  const updates: string[] = [];
  const params: any[] = [];
