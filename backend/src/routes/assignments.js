...
import { errorResponse, successResponse, verifyJWT } from '../utils/helpers.js';
...
function checkMagicBytes(buffer, expectedType) {
    ...
    if (!expected)
        return true; // Skip check for complex types
...
assignments.post('/', authMiddleware, adminOnly, async (c) => {
    ...
    const { title, description, class_id, due_date, max_file_size = 10485760, // 10MB
    allowed_types = 'image/*,video/*,application/pdf', max_attempts = 1 } = body;
    if (!title || !class_id) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'edu')
  `).bind(title, description || null, class_id, due_date || null, max_file_size, allowed_types, max_attempts, user.id, classRow.organizer_uuid || null, classRow.program_uuid || null, classRow.level_uuid || null, classRow.custom_field_payload || null, classRow.override_payload || null).run();
...
assignments.delete('/:id', authMiddleware, adminOnly, async (c) => {
    ...
    const submissions = await db.prepare('SELECT r2_key FROM assignment_submissions WHERE assignment_id = ?').bind(id).all();
    // Delete R2 objects
...
assignments.get('/student', studentAuth, async (c) => {
  ...
  `).bind(student.id, student.id, student.id, ...classIds).all();
    ...
    const assignmentList = results.results.map((a) => ({
        ...a,
...
assignments.post('/:id/submit', studentAuth, strictRateLimiter, async (c) => {
  ...
  `).bind(id, student.id).first();
    ...
    if (isBlockedFile(fileObj.name)) {
        return errorResponse('Loại file không được phép', 400);
    }
    // 8. Check file size
    if (fileObj.size > assignment.max_file_size) {
    ...
    if (!checkMagicBytes(buffer, fileObj.type)) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(id, student.id, attempt, r2Key, fileObj.name, fileObj.size, fileObj.type).run();
    return successResponse({
