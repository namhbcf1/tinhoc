...
const assignments = new Hono<{ Bindings: Env; Variables: { user: JWTPayload; student: any; isAdmin: boolean } }>();
...
assignments.delete('/:id', authMiddleware, adminOnly, async (c) => {
    ...
    for (const sub of submissions.results) {
        try {
            await r2.delete((sub as any).r2_key);
        } catch (e) {
            console.error('Failed to delete R2 object:', (sub as any).r2_key, e);
        }
    }
...
assignments.get('/student', studentAuth, async (c) => {
    const db = c.env.DB;
    const student = c.get('student') as any;
  ...
  `).bind(student.id).all();
    ...
    const classIds = enrollments.results.map((e: any) => e.online_class_id);
  ...
  `).bind(student.id, student.id, student.id, ...classIds).all();
    ...
    const assignmentList = results.results.map((a: any) => ({
        ...a,
