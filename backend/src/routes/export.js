...
import { getExamRegistrations, getExamRegistrationsForExport, } from '../db/attendance-queries.js';
import { normalizeBirthPlaceValue } from '../utils/birth-place.js';
const exportRoute = new Hono();
const EXAM_EXPORT_SCOPE_LABELS = {
...
function normalizeStudentsForExport(students) {
    return (students || []).map((student) => normalizeStudentForExport(student));
}
function resolveExamExportScope(rawScope) {
    if (!rawScope)
        return 'approved';
    if (rawScope === 'approved' || rawScope === 'all')
        return rawScope;
    return null;
}
...
function cleanStudentPlace(value) {
...
function cleanStudentWorkplace(value) {
    return toUpperVi(value);
}
function cleanStudentAddress(value) {
    return toUpperVi(value);
}
function getStudentExportValue(student, field) {
    if (field === 'gioi_tinh')
        return normalizeGenderLabel(student.gioi_tinh);
    if (field === 'noi_sinh')
        return cleanStudentPlace(student.noi_sinh);
    if (field === 'don_vi_cong_tac')
        return cleanStudentWorkplace(student.don_vi_cong_tac);
    if (field === 'dia_chi')
        return cleanStudentAddress(student.dia_chi);
    if (field === 'email')
...
function isTestStudentRecord(student) {
    ...
    const isLegacyTestCode = (value) => /^\d{3,4}$/.test(value) && Number(value) >= 1 && Number(value) <= 19;
    return (fullName.startsWith('test hoc vien') ||
        email.endsWith('@student.local') ||
        cccd.startsWith('test') ||
        isLegacyTestCode(cccd) ||
        isLegacyTestCode(fullName));
}
function excludeTestStudents(students) {
    return students.filter((student) => !isTestStudentRecord(student));
}
function buildExamExportFilename(examName) {
...
function ensureWorksheetCell(worksheet, address, value, type = 's') {
...
function applyCellStyle(worksheet, address, style) {
    const cell = ensureWorksheetCell(worksheet, address, worksheet[address]?.v ?? '', worksheet[address]?.t || 's');
    cell.s = style;
}
function setWorksheetMerges(worksheet, merges) {
    worksheet['!merges'] = merges;
}
function setWorksheetColumns(worksheet, columns) {
    worksheet['!cols'] = columns;
}
function setWorksheetRows(worksheet, rows) {
    worksheet['!rows'] = rows;
}
function setWorksheetFreeze(worksheet, freeze) {
...
function detectExamListTemplateName(input) {
    const templateTokens = [input.templateName, input.templateDisplayName]
        ...
        .filter(Boolean);
    if (templateTokens.some((value) => value.includes('VANTRANG') ||
    ...
    if (tokens.some((value) => value.includes('PTIT') ||
...
function formatExamDateLine(value) {
...
function clearWorksheetValuesFromRow(worksheet, startRow) {
...
function writePtitExamListTemplate(worksheet, examInfo, students, dataStartRow) {
...
function writeVeptExamListTemplate(worksheet, examInfo, students, dataStartRow) {
...
function buildVanTrangFullTitle(examInfo) {
...
function getVanTrangFullColumns() {
    return [
        { header: 'STT', width: 6, value: (_s, index) => index + 1, center: true },
        { header: 'Họ', width: 18, value: (s) => toUpperVi(s.ho || '') },
        { header: 'Tên đệm', width: 18, value: (s) => toUpperVi(s.ten_dem || '') },
        { header: 'Tên', width: 12, value: (s) => toUpperVi(s.ten || '') },
        { header: 'Họ và tên', width: 24, value: (s) => toUpperVi(s.ho_ten_full || [s.ho, s.ten_dem, s.ten].filter(Boolean).join(' ')) },
        { header: 'Ngày sinh', width: 14, value: (s) => formatDateVN(s.ngay_sinh), center: true },
        { header: 'Giới tính', width: 10, value: (s) => normalizeGenderLabel(s.gioi_tinh), center: true },
        { header: 'Dân tộc', width: 12, value: (s) => toUpperVi(s.dan_toc || ''), center: true },
        { header: 'CCCD', width: 18, value: (s) => s.cccd || '', center: true },
        { header: 'Ngày cấp CCCD', width: 14, value: (s) => formatDateVN(s.ngay_cap_cccd), center: true },
        { header: 'SĐT', width: 14, value: (s) => s.sdt || '', center: true },
        { header: 'Email', width: 26, value: (s) => normalizeEmail(s.email || '') },
        { header: 'Nơi sinh', width: 20, value: (s) => cleanStudentPlace(s.noi_sinh) },
        { header: 'Địa chỉ', width: 34, value: (s) => cleanStudentAddress(s.dia_chi) },
        { header: 'Đơn vị công tác', width: 24, value: (s) => cleanStudentWorkplace(s.don_vi_cong_tac) },
    ];
}
function writeVanTrangFullExamListTemplate(worksheet, examInfo, students) {
    const titleStyle = {
...
function applyMappedTemplate(worksheet, template, examInfo, students) {
    ...
    students.forEach((student, index) => {
        ...
        Object.entries(mapping).forEach(([field, colLetter]) => {
            ...
            if (field === 'stt')
                value = index + 1;
            else if (field === 'ho_ten')
                value = toUpperVi(student.ho_ten_full || [student.ho, student.ten_dem, student.ten].filter(Boolean).join(' '));
            else if (field === 'ngay_sinh')
                value = formatDateVN(student.ngay_sinh);
            else if (field === 'ma_sv')
                value = student.cccd || student.id || '';
            else if (field === 'ho_so')
                value = '';
            else
                value = getStudentExportValue(student, field);
            ensureWorksheetCell(worksheet, cellAddress, value, typeof value === 'number' ? 'n' : 's');
        });
    });
    return true;
}
function sortStudentsForExamList(students) {
    students.sort((a, b) => {
...
async function loadExamListExportContext(db, examId, scope = 'approved') {
    ...
    if (!examInfo)
        return null;
    const students = normalizeStudentsForExport(await getExamRegistrationsForExport(db, examId, scope));
...
function buildVeptExamListPreview(examInfo, students) {
    return {
        kind: 'vept',
...
function buildPtitExamListPreview(examInfo, students) {
    return {
        kind: 'exam-list',
...
function buildVanTrangFullExamListPreview(examInfo, students, scopeLabel) {
    ...
    return {
        ...
        rows: students.map((student, index) => columns.map((column) => column.value(student, index))),
    };
}
function buildExamListPreviewData(examInfo, students, resolvedTemplateName, scopeLabel) {
...
exportRoute.get('/class/:class_id', async (c) => {
    try {
        ...
        const registrations = normalizeStudentsForExport(excludeTestStudents(await getRegistrationsByClass(c.env.DB, classId)));
        // 3. Tạo workbook
...
exportRoute.get('/class/:class_id/csv', async (c) => {
    try {
        ...
        const csvRows = registrations.map((reg, index) => {
            ...
            const trangThai = statusMap[reg.status] || toUpperVi(reg.status || '');
            return [
                index + 1,
                reg.so_phach || '',
                reg.cccd || '',
...
exportRoute.get('/exam/:exam_id', async (c) => {
    try {
        ...
        const examInfo = await c.env.DB.prepare('SELECT * FROM exam_schedules WHERE id = ?').bind(examId).first();
        if (!examInfo) {
            return errorResponse('Kỳ thi không tồn tại', 404);
        }
        // 2. Lấy đúng danh sách đã duyệt như màn hình admin (/exam-schedules/:id/students)
        const students = normalizeStudentsForExport(await getExamRegistrations(c.env.DB, examId));
        // 3. Tạo workbook
        const workbook = XLSX.utils.book_new();
        // Parse ngày thi
...
exportRoute.get('/exam/:exam_id/exam-list/preview', async (c) => {
    try {
        ...
        const preview = buildExamListPreviewData(context.examInfo, context.students, context.resolvedTemplateName, context.scopeLabel);
        return c.json({
            success: true,
            data: {
                ...preview,
