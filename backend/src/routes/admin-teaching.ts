...
const adminTeaching = new Hono<{ Bindings: Env; Variables: { user: JWTPayload } }>();
...
adminTeaching.get('/my-classes', async (c) => {
  try {
    ...
    const classes = await getAdminClasses(c.env.DB, user.id as number);
    return jsonResponse({
      success: true,
...
adminTeaching.get('/my-schedule', async (c) => {
  try {
    ...
    const schedule = await getAdminSchedule(c.env.DB, user.id as number, week_start);
    return jsonResponse({
      success: true,
...
adminTeaching.get('/my-exams', async (c) => {
  try {
    const user = c.get('user');
    const exams = await getAdminExams(c.env.DB, user.id as number);
    return jsonResponse({
      success: true,
      data: exams.results || [],
    });
