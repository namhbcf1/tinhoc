// backend/src/routes/exam-taking.js
...
examTaking.get('/my-history', authMiddleware, async (c) => {
  ...
  try {
    ...
    const attempts = rows.results.map(a => ({
      ...a,
...
examTaking.post('/:id/start', authMiddleware, async (c) => {
  ...
  try {
    const examRow = await db.prepare(`SELECT id, status, duration FROM exams WHERE id = ?`).bind(id).first();
...
examTaking.get('/:id/data', authMiddleware, async (c) => {
  ...
  try {
    const examRow = await db.prepare('SELECT * FROM exams WHERE id = ?').bind(id).first();
    if (!examRow) return errorResponse('Exam not found', 404);
    ...
    const sectionsWithData = sections.results.map(s => ({
      ...s,
      groups: groups.results.filter(g => g.section_id === s.id).map(g => groupMap[g.id]),
...
examTaking.post('/:id/answers', authMiddleware, async (c) => {
  ...
  try {
    ...
    ).bind(attemptId, user.id, id).first();
...
examTaking.post('/:id/submit', authMiddleware, async (c) => {
  ...
  try {
    ...
    const sectionScores = { LISTENING: 0, READING: 0, WRITING: 0, SPEAKING: 0 };
