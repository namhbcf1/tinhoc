...
import { findStudentByCCCD, getClassById, findRegistration, createRegistration, updateRegistrationStatus, getRegistrationsByClass, getStudentRegistrations, updateSoPhach, deleteRegistration, } from '../db/queries.js';
...
registrations.post('/', async (c) => {
    try {
        const { cccd, class_id } = await c.req.json();
        if (!cccd || !class_id) {
        ...
        const classInfo = await getClassById(c.env.DB, class_id);
        if (!classInfo) {
        ...
        // 4. Tạo đăng ký
        const result = await createRegistration(c.env.DB, student.id, class_id);
        if (!result.success) {
...
registrations.get('/', async (c) => {
    try {
        ...
        // Tìm sinh viên
        const student = await findStudentByCCCD(c.env.DB, studentCCCD);
        if (!student) {
            return errorResponse('Sinh viên không tồn tại', 404);
        }
        // Lấy danh sách đăng ký
        const list = await getStudentRegistrations(c.env.DB, student.id);
        return jsonResponse({
            success: true,
            data: list,
            count: list.length,
...
registrations.get('/class/:class_id', async (c) => {
    try {
        ...
        const list = await getRegistrationsByClass(c.env.DB, classId);
        return jsonResponse({
            success: true,
            data: list,
            count: list.length,
...
registrations.put('/:id/status', async (c) => {
    try {
        ...
        const result = await updateRegistrationStatus(c.env.DB, id, status, payment_status // Giữ lại để tương thích nhưng không sử dụng
        );
        if (!result.success) {
...
registrations.put('/:id/so-phach', async (c) => {
    try {
        const id = parseInt(c.req.param('id'));
        const { so_phach } = await c.req.json();
