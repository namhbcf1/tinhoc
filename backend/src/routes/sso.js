...
import { createSsoHandoff, exchangeSsoHandoff, getActiveSessionBySid, issueSessionToken, } from '../lib/auth/session-broker.js';
import { findAdminByUsername, findAdminByTeacherCode, updateAdminLastLogin, promoteLegacyTeacherAdmin, } from '../db/admin-queries.js';
...
sso.post('/handoffs', authMiddleware, async (c) => {
    try {
        ...
        const handoff = await createSsoHandoff(c.env.DB, c.env, session, targetApp, body?.return_to || body?.returnTo);
        return jsonResponse({
            success: true,
            ...handoff,
        });
...
sso.post('/direct-login', async (c) => {
    ...
    if (type === 'student') {
        ...
        if (type === 'teacher') {
            ...
            const teacher = await findAdminByTeacherCode(c.env.DB, teacherCode);
