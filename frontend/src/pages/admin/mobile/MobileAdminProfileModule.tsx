...
export default function MobileAdminProfileModule({ admin, onUpdate }) {
    ...
    return (
        <div className="min-h-screen pb-24">
            ...
            <div className="mx-[var(--admin-mobile-page-x,14px)] mt-3 overflow-hidden rounded-[28px] border border-[rgba(200,169,106,0.18)] bg-[radial-gradient(circle_at_90%_0%,rgba(200,169,106,0.26),transparent_32%),linear-gradient(135deg,#132238_0%,#0b1728_62%,#315b80_100%)] px-3 pb-5 pt-4 text-white shadow-[0_24px_58px_-36px_rgba(11,23,40,0.70)]">
                <div className="flex items-center gap-2">
                    <div className="flex h-16 w-16 items-center justify-center rounded-[22px] bg-[linear-gradient(135deg,#fffaf1,#c8a96a)] text-base font-black text-[var(--admin-ink)] shadow-[0_18px_34px_-22px_rgba(200,169,106,0.72)]">
                        {admin?.full_name?.charAt(0)?.toUpperCase() || 'A'}
                    </div>
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[rgba(200,169,106,0.86)]">Admin console</p>
                        <h2 className="truncate text-sm font-black tracking-[-0.04em] text-white">{admin?.full_name || 'Tài khoản admin'}</h2>
            ...
            <div className="sticky top-[calc(var(--mb-header-height)+10px)] z-10 mx-[var(--admin-mobile-page-x,14px)] mt-3 rounded-[22px] border border-[rgba(19,34,56,0.10)] bg-[rgba(255,250,241,0.88)] p-1 shadow-[0_16px_32px_-28px_rgba(19,34,56,0.42)] backdrop-blur-xl">
                <div className="grid grid-cols-2 gap-1">
                    <button
                        ...
                        className={`rounded-[18px] px-3 py-2 text-sm font-black transition ${activeTab === 'profile' ? 'bg-[var(--admin-ink)] text-[var(--admin-champagne)] shadow-[0_14px_26px_-20px_rgba(19,34,56,0.72)]' : 'text-[var(--admin-text-muted)]'}`}
                    >
                        Thông tin
                    </button>
                    <button
                        onClick={() => setActiveTab('password')}
                        className={`rounded-[18px] px-3 py-2 text-sm font-black transition ${activeTab === 'password' ? 'bg-[var(--admin-ink)] text-[var(--admin-champagne)] shadow-[0_14px_26px_-20px_rgba(19,34,56,0.72)]' : 'text-[var(--admin-text-muted)]'}`}
                    >
                        Mật khẩu
                    </button>
                </div>
            ...
            <div className="px-[var(--admin-mobile-page-x,14px)] py-2.5">
                {activeTab === 'profile' ? (
                    <form onSubmit={handleProfileSubmit} className="space-y-2">
                        <section className="rounded-[24px] border border-[rgba(19,34,56,0.11)] bg-[linear-gradient(180deg,rgba(255,250,241,0.98),rgba(247,241,231,0.88))] p-3 shadow-[0_20px_42px_-34px_rgba(19,34,56,0.36)]">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex w-full items-center justify-center gap-2 rounded-[20px] bg-[var(--admin-ink)] px-3 py-2.5 text-sm font-black text-[var(--admin-champagne)] shadow-[0_18px_34px_-24px_rgba(19,34,56,0.62)] transition-transform active:scale-[0.98] disabled:opacity-60"
                        >
                            <Save size={16} />
        ...
        <label className="block">
            ...
            <div className="relative">
                <input
                    ...
                    className="w-full rounded-[18px] border border-[rgba(19,34,56,0.12)] bg-[rgba(255,250,241,0.96)] px-3 py-2 pr-11 text-sm font-semibold text-[var(--admin-ink)] outline-none transition focus:border-[rgba(200,169,106,0.56)] focus:ring-4 focus:ring-[rgba(200,169,106,0.15)]"
                />
                <button
                    type="button"
                    onClick={onToggle}
